...
#include <nationalstonegroup/crypto/AES.h>
#include <nationalstonegroup/crypto/ECIES.h>
#include <nationalstonegroup/crypto/ECDSA.h>
...
Blob RippleAddress::getPasswordCipher(const Blob& passBlob, const Blob& public_key)
{
	...
	Blob public_blob;
	public_blob.resize(public_key.size());
	memcpy(&(public_blob.front()), public_key.data(), public_key.size());
	RippleAddress naAccountPublic;
	naAccountPublic.setAccountPublic(public_blob);

	RippleAddress naSeed;
	//naSeed.setSeedGeneric("masterpassphrase");
	naSeed.setSeedRandom();
	RippleAddress   generator = RippleAddress::createGeneratorPublic(naSeed);
	RippleAddress   rootAccountPublic = RippleAddress::createAccountPublic(generator, 0);
	RippleAddress   rootAccountPrivate = RippleAddress::createAccountPrivate(generator, naSeed, 0);

	Blob vucTextCipher = rootAccountPrivate.accountPrivateEncrypt(naAccountPublic, passBlob);
	//combine with random publickey ahead
	Blob finalCipher;
	finalCipher.resize(rootAccountPublic.size());
	memcpy(&(finalCipher.front()), rootAccountPublic.data(), rootAccountPublic.size());
	finalCipher.insert(finalCipher.end(), vucTextCipher.begin(), vucTextCipher.end());
	return finalCipher;
}

Blob RippleAddress::decryptPassword(const Blob& cipherBlob, const Blob& secret_key)
{
	//get public key
	RippleAddress naSeed;
	//naSeed.setSeedGeneric("masterpassphrase");
	...
	RippleAddress   generator = RippleAddress::createGeneratorPublic(naSeed);
	RippleAddress   rootAccountPublic = RippleAddress::createAccountPublic(generator, 0);
	Blob public_key(cipherBlob.begin(), cipherBlob.begin() + rootAccountPublic.size());
	rootAccountPublic.setAccountPublic(public_key);

	//truncate real cipher
	Blob realCipher(cipherBlob.begin() + rootAccountPublic.size(),cipherBlob.end());

	RippleAddress userprivate;
	Blob secret_blob;
	secret_blob.resize(secret_key.size());
	memcpy(&(secret_blob.front()), secret_key.data(), secret_key.size());
	userprivate.setAccountPrivate(secret_blob);

	Blob  secretDisencrypt = userprivate.accountPrivateDecrypt(rootAccountPublic, realCipher);
