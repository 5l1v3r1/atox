...
#include <nationalstonegroup/crypto/ECDSACanonical.h>
...
public:
    ...
    */
    ...
    // Decrypt a message.Using ECIES algorithm.
    Blob accountPrivateDecrypt (
        RippleAddress const& naPublicFrom, Blob const& vucCipherText) const;

	//static Blob getPasswordCipher(const Blob& passBlob, const PublicKey& public_key);
	//static Blob decryptPassword(const Blob& rawBlob, const SecretKey& secret_key);
    static Blob getPasswordCipher(const Blob& passBlob, const Blob& public_key);
