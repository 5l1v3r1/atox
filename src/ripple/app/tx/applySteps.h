...
struct TxConsequences
{
    ...
    WFNAmount const potentialSpend;

    /// Constructor
    TxConsequences(ConsequenceCategory const category_,
        WFNAmount const fee_, WFNAmount const spend_)
        : category(category_)
        , fee(fee_)
...
/** Determine the WFN balance consequences if a transaction
    consumes the maximum WFN allowed.
