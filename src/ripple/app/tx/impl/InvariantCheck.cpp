    ...
    std::shared_ptr <SLE const> const& after)
{
    if(before)
    {
        ...
        {
        case ltACCOUNT_ROOT:
            drops_ -= (*before)[sfBalance].wfn().drops();
            break;
        case ltPAYCHAN:
            drops_ -= ((*before)[sfAmount] - (*before)[sfBalance]).wfn().drops();
            break;
        case ltESCROW:
            drops_ -= (*before)[sfAmount].wfn().drops();
            break;
        default:
            break;
        }
    ...
    if(after)
    {
        ...
        {
        case ltACCOUNT_ROOT:
            drops_ += (*after)[sfBalance].wfn().drops();
            break;
        case ltPAYCHAN:
            if (! isDelete)
                drops_ += ((*after)[sfAmount] - (*after)[sfBalance]).wfn().drops();
            break;
        case ltESCROW:
            if (! isDelete)
                drops_ += (*after)[sfAmount].wfn().drops();
            break;
        default:
            break;
        }
...
bool
WFNNotCreated::finalize(STTx const& tx, TER /*tec*/, beast::Journal const& j)
{
    auto fee = tx.getFieldAmount(sfFee).wfn().drops();
    if(-1*fee <= drops_ && drops_ <= 0)
        return true;

    JLOG(j.fatal()) << "Invariant failed: WFN net change was " << drops_ <<
        " on a fee of " << fee;
    return false;
}
    ...
    std::shared_ptr <SLE const> const& after)
{
    auto isBad = [](STAmount const& balance)
    {
        ...
        auto const drops = balance.wfn().drops();
...
bool
WFNBalanceChecks::finalize(STTx const&, TER, beast::Journal const& j)
{
    if (bad_)
    {
        JLOG(j.fatal()) << "Invariant failed: incorrect account WFN balance";
        return false;
    }
    ...
    std::shared_ptr <SLE const> const& after)
{
    auto isBad = [](STAmount const& pays, STAmount const& gets)
    {
        ...
        // Can't have an WFN to WFN offer:
    ...
    std::shared_ptr <SLE const> const& after)
{
    auto isBad = [](STAmount const& amount)
    {
        ...
        if (amount.wfn().drops() <= 0)
            return true;

        if (amount.wfn().drops() >= SYSTEM_CURRENCY_START)
            return true;
    ...
    std::shared_ptr <SLE const> const& after)
{
    if (after && after->getType() == ltRIPPLE_STATE)
    {
        ...
        wfnTrustLine_ =
            after->getFieldAmount (sfLowLimit).issue() == wfnIssue() ||
            after->getFieldAmount (sfHighLimit).issue() == wfnIssue();
    }
}

bool
NoWFNTrustLines::finalize(STTx const&, TER, beast::Journal const& j)
{
    if (! wfnTrustLine_)
        return true;

    JLOG(j.fatal()) << "Invariant failed: an WFN trust line was created";
    return false;
}
