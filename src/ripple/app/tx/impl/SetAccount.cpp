...
SetAccount::preflight (PreflightContext const& ctx)
{
    ...
    // DisallowWFN
    //
    bool bSetDisallowWFN   = (uTxFlags & tfDisallowWFN) || (uSetFlag == asfDisallowWFN);
    bool bClearDisallowWFN = (uTxFlags & tfAllowWFN) || (uClearFlag == asfDisallowWFN);

    if (bSetDisallowWFN && bClearDisallowWFN)
    {
        JLOG(j.trace()) << "Malformed transaction: Contradictory flags set.";
        return temINVALID_FLAG;
    }
...
SetAccount::doApply ()
{
    ...
    // DisallowWFN
    //
    if (bSetDisallowWFN && !(uFlagsIn & lsfDisallowWFN))
    {
        JLOG(j_.trace()) << "Set lsfDisallowWFN.";
        uFlagsOut |= lsfDisallowWFN;
    }

    if (bClearDisallowWFN && (uFlagsIn & lsfDisallowWFN))
    {
        JLOG(j_.trace()) << "Clear lsfDisallowWFN.";
        uFlagsOut &= ~lsfDisallowWFN;
    }
