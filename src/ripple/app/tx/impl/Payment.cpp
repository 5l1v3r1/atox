...
Payment::calculateMaxSpend(STTx const& tx)
{
    if (tx.isFieldPresent(sfSendMax))
    {
        ...
        return sendMax.native() ? sendMax.wfn() : beast::zero;
    }
    /* If there's no sfSendMax in WFN, and the sfAmount isn't
    in WFN, then the transaction can not send WFN. */
    auto const& saDstAmount = tx.getFieldAmount(sfAmount);
    return saDstAmount.native() ? saDstAmount.wfn() : beast::zero;
}
...
Payment::preflight (PreflightContext const& ctx)
{
    ...
    // isZero() is WFN.  FIX!
    bool const bWFNDirect = uSrcCurrency.isZero () && uDstCurrency.isZero ();
    ...
    if (bWFNDirect && bMax)
    {
        ...
        JLOG(j.trace()) << "Malformed transaction: " <<
            "SendMax specified for WFN to WFN.";
        return temBAD_SEND_WFN_MAX;
    }
    if (bWFNDirect && bPaths)
    {
        // WFN is sent without paths.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "Paths specified for WFN to WFN.";
        return temBAD_SEND_WFN_PATHS;
    }
    if (bWFNDirect && partialPaymentAllowed)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "Partial payment specified for WFN to WFN.";
        return temBAD_SEND_WFN_PARTIAL;
    }
    if (bWFNDirect && limitQuality)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "Limit quality specified for WFN to WFN.";
        return temBAD_SEND_WFN_LIMIT;
    }
    if (bWFNDirect && !defaultPathsAllowed)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "No ripple direct specified for WFN to WFN.";
        return temBAD_SEND_WFN_NO_DIRECT;
    }
...
Payment::doApply ()
{
    ...
    else
    {
        ...
        auto const mmm = std::max(reserve,
            ctx_.tx.getFieldAmount (sfFee).wfn ());

        if (mPriorBalance < saDstAmount.wfn () + mmm)
        {
            // Vote no. However the transaction might succeed, if applied in
            // a different order.
            JLOG(j_.trace()) << "Delay transaction: Insufficient funds: " <<
                " " << to_string (mPriorBalance) <<
                " / " << to_string (saDstAmount.wfn () + mmm) <<
                " (" << to_string (reserve) << ")";
