...
#include <nationalstonegroup/app/storage/TableStorageItem.h>
...
preflight1 (PreflightContext const& ctx)
{
    ...
    if (!fee.native () || fee.negative () || !isLegalAmount (fee.wfn ()))
    {
...
Transactor::checkFee (PreclaimContext const& ctx, std::uint64_t baseFee)
{
	...
	if (ctx.tx.isNSchainBaseType())
	{
		int wfnDrops = 1000000;
		...
		auto extraAmount = new WFNAmount((int)(wfnDrops * multiplier));
		feeDue += *extraAmount;
	}
...
TER Transactor::payFee ()
{
    ...
    // VFALCO Should we call view().rawDestroyWFN() here as well?
...
TER Transactor::apply ()
{
    ...
    if (sle)
    {
        mPriorBalance   = STAmount ((*sle)[sfBalance]).wfn ();
...
void
Transactor::claimFee (WFNAmount& fee, TER terResult, std::vector<uint256> const& removedOffers)
{
    ctx_.discard();
    ...
    auto const balance = txnAcct->getFieldAmount (sfBalance).wfn ();
...
Transactor::operator()()
{
    ...
    auto fee = ctx_.tx.getFieldAmount(sfFee).wfn ();
