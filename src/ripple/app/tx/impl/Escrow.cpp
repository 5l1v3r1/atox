...
/*
    ...
    of WFN in its own ledger entry, until the escrow process
    ...
    sequestered WFN. If the escrow is, instead, canceled,
    the account which created the escrow will receive the
    sequestered WFN back instead.
...
EscrowCreate::doApply()
{
    ...
    {
        auto const balance = STAmount((*sle)[sfBalance]).wfn();
        ...
        if (balance < reserve + STAmount(ctx_.tx[sfAmount]).wfn())
            return tecUNFUNDED;
    }
    ...
    // Check destination account
    {
        ...
        if ((*sled)[sfFlags] & lsfDisallowWFN)
            return tecNO_TARGET;
    }
