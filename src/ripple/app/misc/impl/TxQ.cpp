    ...
    TxQ::Setup const& setup)
{
    ...
    // happen.
    return mulDiv(tx[sfFee].wfn().drops(),
        baseRefLevel,
        ...
        ApplyFlags flags, beast::Journal j)
{
    ...
    struct MultiTxn
    {
        ...
        WFNAmount fee = beast::zero;
        WFNAmount potentialSpend = beast::zero;
        ...
        if (sle)
        {
            ...
            if (multiTxn)
            {
                /* Check if the total fees in flight are greater
                    ...
                    Assume: Minimum account reserve is 20 WFN.
                    Example 1: If I have 1,000,000 WFN, I can queue
                        a transaction with a 1,000,000 WFN fee. In
                        the meantime, some other transaction may
                        lower my balance (eg. taking an offer). When
                        the transaction executes, I will either
                        spend the 1,000,000 WFN, or the transaction
                        will get stuck in the queue with a
                        `terINSUF_FEE_B`.
                    Example 2: If I have 1,000,000 WFN, and I queue
                        10 transactions with 0.1 WFN fee, I have 1 WFN
                        in flight. I can now queue another tx with a
                        999,999 WFN fee. When the first 10 execute,
                        ...
                        999,999 WFN, or get stuck in the queue.
                    Example 3: If I have 1,000,000 WFN, and I queue
                        7 transactions with 3 WFN fee, I have 21 WFN
                ...
                */
                auto const balance = (*sle)[sfBalance].wfn();
                auto totalFee = multiTxn->fee;
                if (multiTxn->includeCurrentFee)
                    totalFee += (*tx)[sfFee].wfn();
                if (totalFee >= balance ||
