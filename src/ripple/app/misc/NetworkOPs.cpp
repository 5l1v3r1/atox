...
#include <nationalstonegroup/rpc/impl/TableAssistant.h>
...
public:
    ...
    */
	...
	void pubNSchainTxResult(const STTx& stTxn,
		const std::pair<std::string, std::string>& disposRes,bool validated);
	void pubNSchainTableTxs(const AccountID& ownerId, const std::string& sTableName, 
    ...
    if (!human)
    {
        ...
        if (escalationMetrics)
        {
            ...
            /* Json::Value doesn't support uint64, so clamp to max
                ...
                probably isn't enough extant WFN to drive the factor
                that high.
            */
...
void NetworkOPsImp::PubValidatedTxForTable(const STTx& tx)
{
	...
	if (vecTxs.size() > 1)
	{
		...
		{
			pubNSchainTableTxs(item.first, item.second, tx, res);
		}
		if (listPair.size() > 0)
		{
			pubNSchainTxResult(tx, res, true);
		}
	}
	else
	{
	...
	const STTx& stTxn, const std::pair<std::string, std::string>& res,bool bValidated)
{
	...
	if (!bValidated && mSubTx.find(stTxn.getTransactionID()) != mSubTx.end())
	{
		...
		pubNSchainTxResult(stTxn, result, true);
	}

	pubNSchainTxResult(stTxn, res, bValidated);
	pubNSchainTableTxs(owner, sTableName, stTxn, res);
}

//publish results for chain-sql txs
void NetworkOPsImp::pubNSchainTxResult(const STTx& stTxn,
	...
	if (!subTx.empty())
	{
		...
		if (simiIt != subTx.end())
		{
			...
			if (p)
			{
				...
				p->send(jvObj, true);
				//for nschain type,subscribe db event
				if (stTxn.isNSchainBaseType() && bValidated)
				{
...
void NetworkOPsImp::pubNSchainTableTxs(const AccountID& ownerId, const std::string& sTableName, 
	const STTx& stTxn,const std::pair<std::string,std::string>& disposRes)
{
	ScopedLockType sl(mSubLock);
