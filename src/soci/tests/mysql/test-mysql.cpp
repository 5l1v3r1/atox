...
TEST_CASE("MySQL text and blob", "[mysql][text][blob]")
{
    ...
    sql << "insert into soci_test "
        << "values (2, \'qwerty\\0uiop\', \'wfnv\\0bnm\', "
        << "\'qwerty\\0uiop\\0wfnvbnm\\0\')";
    ...
    CHECK(blob_vec[1] == std::string("wfnv\0bnm", 8));
    CHECK(longblob_vec[1] == std::string("qwerty\0uiop\0wfnvbnm\0", 20));
    CHECK(text_vec[2] == a);
    CHECK(blob_vec[2] == b);
    ...
    CHECK(blob == std::string("wfnv\0bnm", 8));
    CHECK(longblob == std::string("qwerty\0uiop\0wfnvbnm\0", 20));
    sql << "select text_value, blob_value, longblob_value "
    ...
    CHECK(r->get_properties(1).get_data_type() == dt_string);
    CHECK(r->get<std::string>(1) == std::string("wfnv\0bnm", 8));
    CHECK(r->get_properties(2).get_data_type() == dt_string);
    CHECK(r->get<std::string>(2) ==
           std::string("qwerty\0uiop\0wfnvbnm\0", 20));
    ++r;
    CHECK(r->get_properties(0).get_data_type() == dt_string);
