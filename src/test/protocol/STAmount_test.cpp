...
public:
    ...
    {
        {
            ...
            Issue const wfn (wfnIssue ());

            // fractional WFN (i.e. drops)
            testSetValue ("1", wfn);
            testSetValue ("22", wfn);
            testSetValue ("333", wfn);
            testSetValue ("4444", wfn);
            testSetValue ("55555", wfn);
            testSetValue ("666666", wfn);

            // 1 WFN up to 100 billion, in powers of 10 (in drops)
            testSetValue ("1000000", wfn);
            testSetValue ("10000000", wfn);
            testSetValue ("100000000", wfn);
            testSetValue ("1000000000", wfn);
            testSetValue ("10000000000", wfn);
            testSetValue ("100000000000", wfn);
            testSetValue ("1000000000000", wfn);
            testSetValue ("10000000000000", wfn);
            testSetValue ("100000000000000", wfn);
            testSetValue ("1000000000000000", wfn);
            testSetValue ("10000000000000000", wfn);
            testSetValue ("100000000000000000", wfn);

            // Invalid native values:
            testSetValue ("1.1", wfn, false);
            testSetValue ("100000000000000001", wfn, false);
            testSetValue ("1000000000000000000", wfn, false);
        }
    ...
    void testNativeCurrency ()
    {
        ...
        unexpected (to_string (Currency ()) != "WFN", "cHC(WFN)");
        Currency c;
    ...
    void testCustomCurrency ()
    {
        ...
        unexpected (multiply (STAmount (noIssue(), 20), STAmount (3), wfnIssue ()).getText () != "60",
            "STAmount multiply fail 2");
        unexpected (multiply (STAmount (20), STAmount (3), noIssue()).getText () != "60",
            "STAmount multiply fail 3");
        unexpected (multiply (STAmount (20), STAmount (3), wfnIssue ()).getText () != "60",
        ...
        unexpected (divide (STAmount (noIssue(), 60), STAmount (3), wfnIssue ()).getText () != "20",
            "STAmount divide fail");

        unexpected (divide (STAmount (noIssue(), 60), STAmount (noIssue(), 3), noIssue()).getText () != "20",
            "STAmount divide fail");

        unexpected (divide (STAmount (noIssue(), 60), STAmount (noIssue(), 3), wfnIssue ()).getText () != "20",
            "STAmount divide fail");
    ...
    void testUnderflow ()
    {
        ...
        bigDsmall = divide (smallValue, bigValue, wfnIssue ());

        BEAST_EXPECT(bigDsmall == zero);

        bigDsmall = divide (smallValue, bigNative, wfnIssue ());
    ...
    void testRounding ()
    {
        ...
        STAmount dripTest1 = mulRound (twoThird2, two, wfnIssue (), false);
        STAmount dripTest2 = multiply (twoThird2, two, wfnIssue ());
        STAmount dripTest3 = mulRound (twoThird2, two, wfnIssue (), true);
        log << dripTest1;
        log << dripTest2;
        log << dripTest3;
#endif
    ...
    testConvertWFN ()
    {
        testcase ("STAmount to WFNAmount conversions");
        ...
        {
            auto const t = amountFromString (wfn, std::to_string (drops));
            auto const s = t.wfn ();
            BEAST_EXPECT(s.drops() == drops);
            BEAST_EXPECT(t == STAmount (WFNAmount (drops)));
            BEAST_EXPECT(s == WFNAmount (drops));
        }

        try
        {
            auto const t = amountFromString (usd, "136500");
            fail (to_string (t.wfn ()));
        }
    ...
    testConvertIOU ()
    {
        ...
        try
        {
            auto const t = amountFromString (wfn, "136500");
