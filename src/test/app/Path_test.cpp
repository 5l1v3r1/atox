...
auto IPE(Issue const& iss)
{
    return STPathElement (
        ...
        wfnAccount (), iss.currency, iss.account);
};
...
public:
    ...
    path_find_consume_all()
    {
...

        {
            ...
            env(offer("carol", WFN(100), USD(100)));
            ...
            std::tie(st, sa, da) = find_paths(env,
                "alice", "bob", Account("bob")["AUD"](-1),
                    boost::optional<STAmount>(WFN(100000000)));
            BEAST_EXPECT(st.empty());
            std::tie(st, sa, da) = find_paths(env,
                "alice", "bob", Account("bob")["USD"](-1),
                    boost::optional<STAmount>(WFN(100000000)));
            BEAST_EXPECT(sa == WFN(100));
    ...
    alternative_path_consume_both()
    {
        ...
        env.fund(WFN(10000), "alice", "bob", gw, gw2);
        env.trust(USD(600), "alice");
    ...
    alternative_paths_limit_returned_paths_to_best_quality()
    {
        ...
        auto const gw2 = Account("gateway2");
        auto const gw2_USD = gw2["USD"];
        env.fund(WFN(10000), "alice", "bob", "carol", "dan", gw, gw2);
        env(rate("carol", 1.1));
    ...
    issues_path_negative_ripple_client_issue_23_larger()
    {
        ...
        using namespace jtx;
        Env env(*this);
        env.fund(WFN(10000), "alice", "bob", "carol", "dan", "edward");
    ...
    // carol holds gateway AUD, sells gateway AUD for WFN
    // bob will hold gateway AUD
    // alice pays bob gateway AUD using WFN
    void
    via_offers_via_gateway()
    {
        ...
        env(offer("carol", WFN(50), AUD(50)));
        env(pay("alice", "bob", AUD(10)), sendmax(WFN(100)), paths(WFN));
    ...
    void path_find_01()
    {
        testcase("Path Find: WFN -> WFN and WFN -> IOU");
        using namespace jtx;
        Env env(*this);
        Account A1 {"A1"};
        Account A2 {"A2"};
        ...
        env.fund(WFN(100000), A1);
        env.fund(WFN(10000), A2);
        env.fund(WFN(1000), A3, G1, G2, G3, M1);
        env.close();
        ...
        env(offer(M1, WFN(10000), G3["ABC"](1000)));

        STPathSet st;
        STAmount sa, da;

        {
            auto const& send_amt = WFN(10);
            std::tie(st, sa, da) = find_paths(env, A1, A2, send_amt,
                boost::none, wfnCurrency());
    ...
    void path_find_02()
    {
        testcase("Path Find: non-WFN -> WFN");
        using namespace jtx;
        Env env(*this);
        Account A1 {"A1"};
        Account A2 {"A2"};
        ...
        env.fund(WFN(1000), A1, A2, G3);
        env.fund(WFN(11000), M1);
        env.close();
        ...
        env(offer(M1, G3["ABC"](1000), WFN(10000)));
        ...
        BEAST_EXPECT(same(st, stpath(G3, IPE(wfnIssue()))));
    }
    ...
    void path_find_03()
    {
        ...
        env.fund(WFN(4999.999898), SRC);
        env.fund(WFN(10846.168060), GATEWAY_DST);
        env.fund(WFN(4291.430036), MONEY_MAKER_1);
        env.fund(WFN(106839.375770), MONEY_MAKER_2);
        env.fund(WFN(1240.997150), A1);
        env.fund(WFN(14115.046893), A2);
        env.fund(WFN(512087.883181), A3);
        env.close();
        ...
        env(offer(MONEY_MAKER_2, WFN(1), GATEWAY_DST["CNY"](1)));
        env(offer(MONEY_MAKER_2, GATEWAY_DST["CNY"](1), WFN(1)));
        env(offer(MONEY_MAKER_2, GATEWAY_DST["CNY"](318000), WFN(53000)));
        env(offer(MONEY_MAKER_2, WFN(209), MONEY_MAKER_2["CNY"](4.18)));
        env(offer(MONEY_MAKER_2, MONEY_MAKER_1["CNY"](990000), WFN(10000)));
        env(offer(MONEY_MAKER_2, MONEY_MAKER_1["CNY"](9990000), WFN(10000)));
        env(offer(MONEY_MAKER_2, GATEWAY_DST["CNY"](8870000), WFN(10000)));
        env(offer(MONEY_MAKER_2, WFN(232), MONEY_MAKER_2["CNY"](5.568)));
        env(offer(A2, WFN(2000), MONEY_MAKER_1["CNY"](66.8)));
        env(offer(A2, WFN(1200), GATEWAY_DST["CNY"](42)));
        env(offer(A2, MONEY_MAKER_1["CNY"](43.2), WFN(900)));
        env(offer(A3, MONEY_MAKER_1["CNY"](2240), WFN(50000)));
        ...
        std::tie(st, sa, da) = find_paths(env, SRC, GATEWAY_DST, send_amt,
            boost::none, wfnCurrency());
        BEAST_EXPECT(equal(da, send_amt));
        BEAST_EXPECT(equal(sa, WFN(288.571429)));
    ...
    void path_find_04()
    {
        ...
        env.fund(WFN(1000), G1BS, G2SW, A1, A2);
        env.fund(WFN(11000), M1);
        env.close();
    ...
    void path_find_05()
    {
        testcase("Path Find: non-WFN -> non-WFN, same currency");
        using namespace jtx;
        Env env(*this);
        Account A1 {"A1"};
        Account A2 {"A2"};
        ...
        env.fund(WFN(1000), A1, A2, A3, G1, G2, G3, G4);
        env.fund(WFN(10000), A4);
        env.fund(WFN(11000), M1, M2);
        env.close();
        ...
        env(offer(M2, WFN(10000), G2["HKD"](1000)));
        env(offer(M2, G1["HKD"](1000), WFN(10000)));
...

        {
            ...
            BEAST_EXPECT(same(st,
                ...
                stpath(M1),
                stpath(M2),
                stpath(IPE(wfnIssue()), IPE(G2["HKD"]))
            ));
        }

        {
            ...
            BEAST_EXPECT(same(st,
                ...
                stpath(G1, IPE(wfnIssue()), IPE(G2["HKD"]))
            ));
        }

        {
            //I4) WFN bridge" --
            //  Source -> AC -> OB to WFN -> OB from WFN -> AC -> Destination
            auto const& send_amt = A2["HKD"](10);
            std::tie(st, sa, da) = find_paths(env, A1, A2, send_amt,
                boost::none, G1["HKD"].currency);
            BEAST_EXPECT(equal(da, send_amt));
            ...
            BEAST_EXPECT(same(st,
                ...
                stpath(G1, IPE(wfnIssue()), IPE(G2["HKD"]), G2)
            ));
        }
    }
    ...
    void path_find_06()
    {
        ...
        env.fund(WFN(11000), M1);
        env.fund(WFN(1000), A1, A2, A3, G1, G2);
        env.close();
