...
struct Regression_test : public beast::unit_test::suite
{
    ...
    void testOffer1()
    {
        ...
        env(offer("alice", USD(10), WFN(10)), require(owners("alice", 1)));
        env(offer("alice", USD(20), WFN(10)), json(R"raw(
                { "OfferSequence" : 2 }
    ...
    {
        testcase("Account balance < fee destroys correct amount of WFN");
        using namespace jtx;
        Env env(*this);
        env.memoize("alice");
        ...
        auto const aliceWFN = 400;
        auto const aliceAmount = WFN(aliceWFN);
        ...
        }
        {
            ...
            BEAST_EXPECT(balance == WFN(0));
        }
        expectedDrops -= aliceWFN * dropsPerWFN<int>::value;
        BEAST_EXPECT(next->info().drops == expectedDrops);
    }
    ...
    void testSecp256r1key ()
    {
        ...
        env.fund(WFN(10000), alice, becky);
