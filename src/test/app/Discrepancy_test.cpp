...
class Discrepancy_test : public beast::unit_test::suite
{
    ...
    void
    testWFNDiscrepancy (std::initializer_list<uint256> fs)
    {
        testcase ("Discrepancy test : WFN Discrepancy");
        ...
        env.fund(WFN(2000), A1);
        env.fund(WFN(1000), A2, A6, A7);
        env.fund(WFN(5000), A3);
        env.fund(WFN(1000000), A4);
        env.fund(WFN(600000), A5);
        env.close();
        ...
        env(offer(A4, WFN(49147), A2["JPY"](34501)));
        env(offer(A5, A3["CNY"](3150), WFN(80086)));
        env(offer(A7, WFN(1233), A6["CNY"](25)));
        env.close();

        test::PathSet payPaths {
            test::Path {A2["JPY"], A2},
            test::Path {WFN, A2["JPY"], A2},
            test::Path {A6, WFN, A2["JPY"], A2} };
...
public:
    ...
    {
        testWFNDiscrepancy ({});
        testWFNDiscrepancy ({featureFlow});
        testWFNDiscrepancy ({featureFlow, fix1373});
        testWFNDiscrepancy ({featureFlow, fix1373, featureFlowCross});
    }
};
