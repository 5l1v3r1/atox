...
wfnMinusFee (jtx::Env const& env, std::int64_t wfnAmount)
{
    ...
    return drops (
        dropsPerWFN<std::int64_t>::value * wfnAmount - feeDrops);
};
...
struct Flow_test : public beast::unit_test::suite
{
    ...
    void testDirectStep (std::initializer_list<uint256> fs)
    {
        ...
        auto const USD = gw["USD"];
        {
            ...
            env.fund (WFN (10000), alice, bob, gw);
        ...
        }
        {
            ...
            env.fund (WFN (10000), alice, bob);
            env (pay (alice, bob, WFN (100)));
            env.require (balance (bob, WFN (10000 + 100)));
            env.require (balance (alice, wfnMinusFee (env, 10000 - 100)));
        }
        {
            // Partial payments
            Env env (*this, with_features(fs));
        ...
        }
        {
            ...
            env.fund (WFN (10000), alice, bob, carol, dan);
        ...
        }
        {
            ...
            env.fund (WFN (10000), alice, bob, carol, dan, erin);
    ...
    void testBookStep (std::initializer_list<uint256> fs)
    {
        ...
        }
        {
            ...
            env (offer (bob, BTC (50), WFN (50)));
            env (offer (bob, WFN (50), USD (50)));

            env (pay (alice, carol, USD (50)), path (~WFN, ~USD),
                sendmax (BTC (50)));
            ...
            env.require (balance (bob, USD (0)));
            env.require (balance (carol, USD (50)));
            BEAST_EXPECT(!isOffer (env, bob, WFN (50), USD (50)));
            BEAST_EXPECT(!isOffer (env, bob, BTC (50), WFN (50)));
        }
        {
            // simple WFN -> USD through offer and sendmax
            Env env (*this, with_features(fs));
            ...
            env.require (balance (bob, wfnMinusFee (env, 10000 + 50)));
            env.require (balance (bob, USD (0)));
            env.require (balance (carol, USD (50)));
            BEAST_EXPECT(!isOffer (env, bob, WFN (50), USD (50)));
        }
        {
            // simple USD -> WFN through offer and sendmax
            Env env (*this, with_features(fs));
            ...
            env (offer (bob, USD (50), WFN (50)));

            env (pay (alice, carol, WFN (50)), path (~WFN),
                 sendmax (USD (50)));

            env.require (balance (alice, USD (0)));
            env.require (balance (bob, wfnMinusFee (env, 10000 - 50)));
            env.require (balance (bob, USD (50)));
            env.require (balance (carol, WFN (10000 + 50)));
            BEAST_EXPECT(!isOffer (env, bob, USD (50), WFN (50)));
        }
        {
        ...
        }
        {
            ...
            env (pay (alice, carol, EUR (1)), path (~WFN, ~EUR),
    ...
    void testTransferRate (std::initializer_list<uint256> fs)
    {
        ...
        }
        {
            ...
            env (pay (alice, carol, USD (50)), path (~USD), sendmax (WFN (50)));
            env.require (
                balance (alice, wfnMinusFee (env, 10000 - 50)),
...

        {
            ...
            env (pay (alice, carol, EUR (40)), path (~USD, ~EUR), sendmax (WFN (40)));
            env.require (
                balance (alice,  wfnMinusFee (env, 10000 - 40)),
                balance (bob, USD (40)),
                balance (bob, EUR (0)),
        ...
        }
        {
            ...
            env.fund (WFN (10000), alice, bob, carol, dan, gw);
            env(rate(gw, 1.25));
...

        {
            ...
            env(rate(gw, 1.25));
            env.trust (USD (1000), alice, bob);
            env (offer (gw, WFN (100), USD (100)));
            env (pay (alice, bob, USD (100)),
                 sendmax (WFN (100)));
            env.require (
                balance (alice, wfnMinusFee(env, 10000-100)),
                balance (bob, USD (100)));
        }
        if (!hasFeature(featureOwnerPaysFee, fs))
        {
            ...
            env(rate(gw, 1.25));
            env.trust (USD (1000), alice, bob);
            env (offer (gw, WFN (125), USD (125)));
            env (pay (alice, bob, USD (100)),
                 sendmax (WFN (200)));
            env.require (
                balance (alice, wfnMinusFee(env, 10000-125)),
                balance (bob, USD (100)));
        }
    }
    ...
    testFalseDry(std::initializer_list<uint256> fs)
    {
        ...
        env.fund (WFN(10000), alice, carol, gw);
        ...
        // Bob has _just_ slightly less than 50 wfn available
        // If his owner count changes, he will have more liquidity.
        // This is one error case to test (when Flow is used).
        // Computing the incomming wfn to the WFN/USD offer will require two
        // recursive calls to the EUR/WFN offer. The second call will return
        // tecPATH_DRY, but the entire path should not be marked as dry. This
        // is the second error case to test (when flowV1 is used).
        env (offer (bob, EUR (50), WFN (50)));
        env (offer (bob, WFN (50), USD (50)));

        env (pay (alice, carol, USD (1000000)), path (~WFN, ~USD),
            sendmax (EUR (500)),
    ...
    testLimitQuality ()
    {
        ...
        for (auto const& d : {-100 * timeDelta, +100 * timeDelta})
        {
            ...
            env (offer (bob, WFN (50), USD (50)));
            env (offer (bob, WFN (100), USD (50)));

            auto expectedResult =
                closeTime < fix1141Time () ? tecPATH_DRY : tesSUCCESS;
            env (pay (alice, carol, USD (100)), path (~USD), sendmax (WFN (100)),
    ...
    static WFNAmount reserve(jtx::Env& env, std::uint32_t count)
    {
    ...
    void testSelfFundedWFNEndpoint (bool consumeOffer, std::initializer_list<uint256> fs)
    {
        // Test that the deferred credit table is not bypassed for
        // WFNEndpointSteps. If the account in the first step is sending WFN and
        // that account also owns an offer that receives WFN, it should not be
        // possible for that step to use the WFN received in the offer as part
        // of the payment.
        testcase("Self funded WFNEndpoint");
        ...
        env(offer(alice, WFN(50000), USD(10)));
        ...
        env(pay(alice, alice, toSend), path(~USD), sendmax(WFN(20000)),
    ...
    void testUnfundedOffer (bool withFix, std::initializer_list<uint256> fs)
    {
        ...
        using namespace jtx;
        {
            ...
            env.fund(WFN(100000), alice, bob, gw);
            ...
            if (withFix)
                BEAST_EXPECT(!isOffer(env, gw, WFN(0), USD(0)));
            else
                BEAST_EXPECT(isOffer(env, gw, WFN(0), USD(0)));
        }
        {
            ...
            env(pay(alice, bob, drops(9000000000)), path(~WFN),
                sendmax(USD(1)), txflags(tfNoRippleDirect));

            if (withFix)
                BEAST_EXPECT(!isOffer(env, gw, USD(0), WFN(0)));
            else
                BEAST_EXPECT(isOffer(env, gw, USD(0), WFN(0)));
        }
    }
    ...
    testReexecuteDirectStep(std::initializer_list<uint256> fs)
    {
        ...
        env(pay(alice, bob, WFN(10000)), path(~WFN), sendmax(USD(100)),
            txflags(tfPartialPayment | tfNoRippleDirect));
    }
    ...
    testRIPD1443(bool withFix)
    {
        ...
        env.fund(WFN(100000000), alice, noripple(bob), carol, gw);
        ...
        env(offer(alice, bob["USD"](1000), WFN(1)));
        env.close();

        env(pay(alice, alice, WFN(1)), path(gw, bob, ~WFN),
        ...
        env(offer(alice, WFN(1000), bob["USD"](1000)));
        env.close();

        env(pay (carol, carol, gw["USD"](1000)), path(~bob["USD"], gw),
            sendmax(WFN(100000)), txflags(tfNoRippleDirect),
            ter(withFix ? tecPATH_DRY : tesSUCCESS));
        env.close();
    ...
    testRIPD1449(bool withFix)
    {
        ...
        // pay alice -> wfn -> USD/bob -> bob -> gw -> alice
        ...
        env.fund(WFN(100000000), alice, bob, carol, gw);
        ...
        env(offer(carol, WFN(1), bob["USD"](1000)));
        env.close();

        env(pay(alice, alice, USD(1000)), path(~bob["USD"], bob, gw),
            sendmax(WFN(1)), txflags(tfNoRippleDirect),
            ter(withFix ? tecPATH_DRY : tesSUCCESS));
        env.close();
    }
    ...
    testSelfPayLowQualityOffer (std::initializer_list<uint256> fs)
    {
        ...
        // WFN than the offering account held.  This unit test reproduces
        // that failing case.
    ...
    void run() override
    {
        ...
        auto testWithFeats = [this](auto&&... fs)
        {
            ...
            testSelfPayment1({fs...});
            testSelfPayment2({fs...});
            testSelfFundedWFNEndpoint(false, {fs...});
            testSelfFundedWFNEndpoint(true, {fs...});
