...
struct NSchain_test : public beast::unit_test::suite
{
    void run() override
    {
        testcase("NSchain test");
        using namespace jtx;
        ...
        //JLOG(logs_->journal("NSchain").trace()) << "1000 times of insert errortimes is"<< errortimes;
        //JLOG(logs_->journal("NSchain").trace()) << "total running time is" << double(end - start) / CLOCKS_PER_SEC;
    }
};

BEAST_DEFINE_TESTSUITE(NSchain,app,ripple);
