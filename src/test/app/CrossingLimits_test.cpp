...
public:
    ...
    testStepLimit(std::initializer_list<uint256> fs)
    {
        ...
        auto const wfnMax = WFN(100000000000);
        auto const gw = Account("gateway");
        auto const USD = gw["USD"];

        env.fund(WFN(100000000), gw, "alice", "bob", "carol", "dan");
        env.trust(USD(1), "bob");
        env(pay(gw, "bob", USD(1)));
        env.trust(USD(1), "dan");
        env(pay(gw, "dan", USD(1)));
        n_offers (env, 2000, "bob", WFN(1), USD(1));
        n_offers (env, 1, "dan", WFN(1), USD(1));

        // Alice offers to buy 1000 WFN for 1000 USD. She takes Bob's first
        ...
        // Carol offers to buy 1000 WFN for 1000 USD. She removes Bob's next
        // 1000 offers as unfunded and hits the step limit.
        env(offer("carol", USD(1000), WFN(1000)),
            require (
    ...
    testCrossingLimit(std::initializer_list<uint256> fs)
    {
        ...
        env.fund(WFN(100000000), gw, "alice", "bob", "carol");
        env.trust(USD(1000), "bob");
        env(pay(gw, "bob", USD(1000)));
        n_offers (env, 1000, "bob", WFN(1), USD(1));

        // Alice offers to buy 1000 WFN for 1000 USD. She takes the first
        ...
        // Carol offers to buy 1000 WFN for 1000 USD. She takes the remaining
    ...
    testStepAndCrossingLimit(std::initializer_list<uint256> fs)
    {
        ...
        env.fund(WFN(100000000), gw, "alice", "bob", "carol", "dan", "evita");
        ...
        n_offers (env, 400, "alice", WFN(1), USD(1));
        n_offers (env, 700, "carol", WFN(1), USD(1));
        n_offers (env, 999, "evita", WFN(1), USD(1));

        // Bob offers to buy 1000 WFN for 1000 USD. He takes all 400 USD from
        // Alice's offers, 1 USD from Carol's and then removes 599 of Carol's
        // offers as unfunded, before hitting the step limit.
        env(offer("bob", USD(1000), WFN(1000)),
            require (
                balance("bob", USD(401)),
        ...
        // Dan offers to buy 900 WFN for 900 USD. He removes all 100 of Carol's
        // offers as unfunded, then takes 850 USD from Evita's, hitting the
        // crossing limit.
        env(offer("dan", USD(900), WFN(900)),
            require (
                balance("dan", USD(850)),
                balance("alice", USD(600)), owners("alice", 1),
