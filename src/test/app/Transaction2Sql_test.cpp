...
#include <nationalstonegroup/app/sql/SQLConditionTree.h>
#include <nationalstonegroup/app/sql/STTx2SQL.h>
...
{\"field\":\"xid\",\"type\":\"int\",\"PK\":1,\"NN\":1}]";
	...
	void test_InsertRecordTransaction() {
		...
		STTx tx(ttSQLSTATEMENT, [this, &hex_table, &keypair](STObject &obj) {
			...
			obj.setFieldU16(sfOpType, 6); // insert one record 
			set_tables(obj);
			std::string raw = "[{\"id\":1,\"name\":\"test1\",\"cash\":100.00,\"comment\":\"wfn\",\"deci\":200.00000,\"datetime\":\"2017/1/11 9:42:54\"},\
...
{\"id\":3,\"name\":\"test3\",\"cash\":300.00,\"comment\":\"u.s\", \"deci\":300.01,\"datetime\":\"2017/06/19 20:42:54\",\"ch\":\"testchar\",\"date_field\":\"2017/06/19\"}]";
	...
	void test_join_select() {
		// create both tables user and order_goods
		{
			...
			s << "insert into t_user (uid, username, password) values(3,'nationalstonegroup','123')";
		...
		// inner join
		{
			...
			std::string expected_resul = "{\"lines\":[{\"buytime\":12333222,\"name\":\"apple\",\"password\":\"123\",\"uid\":2,\"username\":\"wangxiaoer\"},{\"buytime\":12333222,\"name\":\"iphone\",\"password\":\"123\",\"uid\":3,\"username\":\"nationalstonegroup\"}],\"status\":\"success\"}";
		...
		// left join
		{
			...
			Json::Value result = txstore_->txHistory(context);
			std::string real_result = Json::jsonAsString(result);
			std::string expected_resul = "{\"lines\":[{\"buytime\":12333222,\"name\":\"apple\",\"password\":\"123\",\"uid\":2,\"username\":\"wangxiaoer\"},{\"buytime\":12333222,\"name\":\"iphone\",\"password\":\"123\",\"uid\":3,\"username\":\"nationalstonegroup\"},{\"buytime\":0,\"name\":\"null\",\"password\":\"123\",\"uid\":1,\"username\":\"jingtian\"}],\"status\":\"success\"}";
			BEAST_EXPECT(boost::iequals(expected_resul, real_result));
		}
	...
	void test_assert_transaction() {
...

		{
			...
			STTx tx(ttSQLSTATEMENT, [this, &hex_table, &keypair](STObject &obj) {
				...
				//std::string raw = "[{\"$or\":[{\"name\":\"nationalstonegroup\"},{\"id\":1}]}, {\"id\":{\"$ge\":1}}]";
				//std::string raw = "[{\"$and\":[{\"name\":\"nationalstonegroup\"},{\"id\":1}]}, {\"id\":{\"$ge\":1}}]";
				//std::string raw = "[{\"$and\":[{\"$or\":[{\"name\":\"nationalstonegroup\"},{\"id\":1}]},{\"id\":10}]}, {\"id\":{\"$ge\":1}}]";
				ripple::Blob blob;
				blob.assign(raw.begin(), raw.end());
				obj.setFieldVL(sfRaw, blob);
	...
	void test_sample_mongodb_json_style() {
		...
		{
			std::string raw_string = "[{\"name\":{\"$nin\":[\"nationalstonegroup\",\"zongxiang\"]}}]";
			Json::Reader reader = Json::Reader();
			Json::Value conditions;
			if (reader.parse(raw_string, conditions) == false) {
				std::cout << "parse error. " << reader.getFormatedErrorMessages() << std::endl;
			...
			std::string result_conditions = result.second.asString();
			std::string expect_conditions = "name not in ('nationalstonegroup','zongxiang')";
			BEAST_EXPECT(result_conditions == expect_conditions);
		}

		{
			std::string raw_string = "[{\"name\":{\"$nin\":[\"nationalstonegroup\"]}}]";
			Json::Reader reader = Json::Reader();
			Json::Value conditions;
			if (reader.parse(raw_string, conditions) == false) {
				std::cout << "parse error. " << reader.getFormatedErrorMessages() << std::endl;
			...
			std::string result_conditions = result.second.asString();
			std::string expect_conditions = "name not in ('nationalstonegroup')";
			BEAST_EXPECT(result_conditions == expect_conditions);
		}
		...
		{
			std::string raw_string = "[{\"name\":\"nationalstonegroup\"}]";
			Json::Reader reader = Json::Reader();
			Json::Value conditions;
			if (reader.parse(raw_string, conditions) == false) {
				std::cout << "parse error. " << reader.getFormatedErrorMessages() << std::endl;
			...
			std::string result_conditions = result.second.asString();
			std::string expect_conditions = "name = 'nationalstonegroup'";
			BEAST_EXPECT(result_conditions == expect_conditions);
		}

		{
			std::string raw_string = "[{\"name\":{\"$regex\":\"/^nationalstonegroup/\"}}]";
			Json::Reader reader = Json::Reader();
			Json::Value conditions;
			if (reader.parse(raw_string, conditions) == false) {
				std::cout << "parse error. " << reader.getFormatedErrorMessages() << std::endl;
			...
			std::string result_conditions = result.second.asString();
			std::string expect_conditions = "name like '%nationalstonegroup'";
			BEAST_EXPECT(result_conditions == expect_conditions);
		}
	...
	void test_keywork() {
		...
		{
			std::string raw_string = "[{\"$and\":[{\"age\":{\"ge\":20}},{\"name\":\"nationalstonegroup\"}]}]";
			Json::Reader reader = Json::Reader();
			Json::Value conditions;
			if (reader.parse(raw_string, conditions) == false) {
				std::cout << "parse error. " << reader.getFormatedErrorMessages() << std::endl;
		...
		{
			std::string raw_string = "[{\"or\":[{\"age\":{\"$ge\":20}},{\"name\":\"nationalstonegroup\"}]}]";
			Json::Reader reader = Json::Reader();
			Json::Value conditions;
			if (reader.parse(raw_string, conditions) == false) {
				std::cout << "parse error. " << reader.getFormatedErrorMessages() << std::endl;
	...
	void test_logicalAnd() {
		...
		std::string expect_ops[] = {"=", "!=", "<", "<=", ">", ">="};
		for (size_t i = 0; i < 6; i++) {
			std::string raw_string = (boost::format("[{\"age\":{\"%1%\":20}, \"name\":{\"%1%\":\"nationalstonegroup\"}}]") 
				%ops[i]).str();
			Json::Reader reader = Json::Reader();
			Json::Value conditions;
			if (reader.parse(raw_string, conditions) == false) {
			...
			std::string result_conditions = result.second.asString();
			std::string expect_conditions = (boost::format("age %1% 20 and name %1% 'nationalstonegroup'") %expect_ops[i]).str();
			BEAST_EXPECT(result_conditions == expect_conditions);
		}
...

		{
			...
			std::string result_conditions = result.second.asString();
			std::string expect_conditions = "age >= 20 and name = 'nationalstonegroup'";
			BEAST_EXPECT(result_conditions == expect_conditions);	
		}
		...
		{
			std::string raw_string = "[{\"$and\":[{\"$or\":[{\"name\":\"nationalstonegroup\"},{\"id\":{\"$ge\":20}}]},{\"age\":20}]}]";
			Json::Reader reader = Json::Reader();
			Json::Value conditions;
			if (reader.parse(raw_string, conditions) == false) {
				std::cout << "parse error. " << reader.getFormatedErrorMessages() << std::endl;
			...
			if(result2.first == 0) {
				std::string result_conditions = result.second.asString();
				std::string expect_conditions = "(name = 'nationalstonegroup' or id >= 20) and age = 20";
				BEAST_EXPECT(result_conditions == expect_conditions);
			}
			else {
				BEAST_EXPECT(result2.first == 0);
	...
	void test_logicalOr() {
		...
		std::string expect_ops[] = { "=", "!=", "<", "<=", ">", ">=" };
		for (size_t i = 0; i < 6; i++) {
			std::string raw_string = (boost::format("[{\"$or\":[{\"age\":{\"%1%\":20}}, {\"name\":{\"%1%\":\"nationalstonegroup\"}}]}]")
				% ops[i]).str();
			Json::Reader reader = Json::Reader();
			Json::Value conditions;
			if (reader.parse(raw_string, conditions) == false) {
			...
			if (result2.first == 0) {
				std::string result_conditions = result.second.asString();
				std::string expect_conditions = (boost::format("age %1% 20 or name %1% 'nationalstonegroup'") % expect_ops[i]).str();
				BEAST_EXPECT(result_conditions == expect_conditions);
			}
			else {
				BEAST_EXPECT(result2.first == 0);
		...
		{
			std::string raw_string = "[{\"age\":{\"$ge\":20}},{\"name\":\"nationalstonegroup\"}]";
			Json::Reader reader = Json::Reader();
			Json::Value conditions;
			if (reader.parse(raw_string, conditions) == false) {
				std::cout << "parse error. " << reader.getFormatedErrorMessages() << std::endl;
			...
			if (result2.first == 0) {
				std::string result_conditions = result.second.asString();
				std::string expect_conditions = "age >= 20 or name = 'nationalstonegroup'";
				BEAST_EXPECT(result_conditions == expect_conditions);
			}
			else {
				BEAST_EXPECT(result2.first == 0);
		...
		{
			std::string raw_string = "[{\"age\":{\"$ge\":20}},{\"name\":{\"$eq\":\"nationalstonegroup\"}}]";
			Json::Reader reader = Json::Reader();
			Json::Value conditions;
			if (reader.parse(raw_string, conditions) == false) {
				std::cout << "parse error. " << reader.getFormatedErrorMessages() << std::endl;
		...
		{
			std::string raw_string = "[{\"age\":{\"$ge\":20,\"$le\":100}},{\"name\":{\"$eq\":\"nationalstonegroup\"}}]";
			Json::Reader reader = Json::Reader();
			Json::Value conditions;
			if (reader.parse(raw_string, conditions) == false) {
				std::cout << "parse error. " << reader.getFormatedErrorMessages() << std::endl;
			...
			if (result2.first == 0) {
				std::string result_conditions = result.second.asString();
				std::string expect_conditions = "(age >= 20 and age <= 100) or name = 'nationalstonegroup'";
				BEAST_EXPECT(result_conditions == expect_conditions);
			}
			else {
				BEAST_EXPECT(result2.first == 0);
		...
		{
			std::string raw_string = "[{\"age\":{\"$ge\":20},\"id\":{\"$ge\":10}},{\"name\":{\"$eq\":\"nationalstonegroup\"}}]";
			Json::Reader reader = Json::Reader();
			Json::Value conditions;
			if (reader.parse(raw_string, conditions) == false) {
				std::cout << "parse error. " << reader.getFormatedErrorMessages() << std::endl;
			...
			if (result2.first == 0) {
				std::string result_conditions = result.second.asString();
				std::string expect_conditions = "(age >= 20 and id >= 10) or name = 'nationalstonegroup'";
				BEAST_EXPECT(result_conditions == expect_conditions);
			}
			else {
				BEAST_EXPECT(result2.first == 0);
	...
	void test_logicRecursion() {
		{
			std::string raw_string = "[{\"$or\":[{\"$and\":[{\"$or\":[{\"age\":10},{\"name\": \"3kx\"}]},{\"name\": \"nationalstonegroup\"}]},{\"id\": 1000}]}]";
			Json::Reader reader = Json::Reader();
			Json::Value conditions;
			if (reader.parse(raw_string, conditions) == false) {
				std::cout << "parse error. " << reader.getFormatedErrorMessages() << std::endl;
			...
			if (result2.first == 0) {
				std::string result_conditions = result.second.asString();
				std::string expect_conditions = "((age = 10 or name = '3kx') and name = 'nationalstonegroup') or id = 1000";
				BEAST_EXPECT(result_conditions == expect_conditions);
			}
			else {
				BEAST_EXPECT(result2.first == 0);
		...
		{
			// 嵌套 2 层
			std::string raw_string = "[{\"$or\": [{\"$or\": [{\"$or\": [{\"age\":10},{\"name\": \"3kx\"}]},{\"name\": \"nationalstonegroup\"}]},{\"$and\": [{\"$or\": [{\"age\":30},{\"name\": \"xxxkkk\"}]},{\"name\": \"zxxx\"}]}]}]";
			Json::Reader reader = Json::Reader();
			Json::Value conditions;
			if (reader.parse(raw_string, conditions) == false) {
				std::cout << "parse error. " << reader.getFormatedErrorMessages() << std::endl;
			...
			if (result2.first == 0) {
				std::string result_conditions = result.second.asString();
				std::string expect_conditions = "((age = 10 or name = '3kx') or name = 'nationalstonegroup') or ((age = 30 or name = 'xxxkkk') and name = 'zxxx')";
				BEAST_EXPECT(result_conditions == expect_conditions);
			}
			else {
				BEAST_EXPECT(result2.first == 0);
		...
		{
			// 嵌套 2 层
			std::string raw_string = "[{\"$or\": [{\"$and\": [{\"$or\": [{\"age\":10},{\"name\": \"3kx\"}]},{\"name\": \"nationalstonegroup\"}]},{\"$and\": [{\"$or\": [{\"age\":30},{\"name\": \"xxxkkk\"}]},{\"name\": \"zxxx\"}]}]}]";
			Json::Reader reader = Json::Reader();
			Json::Value conditions;
			if (reader.parse(raw_string, conditions) == false) {
				std::cout << "parse error. " << reader.getFormatedErrorMessages() << std::endl;
			...
			if (result2.first == 0) {
				std::string result_conditions = result.second.asString();
				std::string expect_conditions = "((age = 10 or name = '3kx') and name = 'nationalstonegroup') or ((age = 30 or name = 'xxxkkk') and name = 'zxxx')";
				BEAST_EXPECT(result_conditions == expect_conditions);
			}
			else {
				BEAST_EXPECT(result2.first == 0);
		...
		{
			// 嵌套 3 层
			std::string raw_string = "[{\"$or\": [{\"$and\": [{\"$or\": [{\"$or\":[{\"id\":100},{\"age\":20},{\"name\":\"ll\"}]},{\"name\": \"3kx\"}]},{\"name\": \"nationalstonegroup\"}]},{\"id\":10}]}]";
			Json::Reader reader = Json::Reader();
			Json::Value conditions;
			if (reader.parse(raw_string, conditions) == false) {
				std::cout << "parse error. " << reader.getFormatedErrorMessages() << std::endl;
			...
			if (result2.first == 0) {
				std::string result_conditions = result.second.asString();
				std::string expect_conditions = "(((id = 100 or age = 20 or name = 'll') or name = '3kx') and name = 'nationalstonegroup') or id = 10";
				BEAST_EXPECT(result_conditions == expect_conditions);
			}
			else {
				BEAST_EXPECT(result2.first == 0);
		...
		{
			// 嵌套 3 层
			std::string raw_string = "[{\"$or\": [{\"$and\": [{\"$or\": [{\"$and\":[{\"id\":100},{\"age\":20},{\"name\":\"ll\"}]},{\"name\": \"3kx\"}]},{\"name\": \"nationalstonegroup\"}]},{\"id\":10}]}]";
			Json::Reader reader = Json::Reader();
			Json::Value conditions;
			if (reader.parse(raw_string, conditions) == false) {
				std::cout << "parse error. " << reader.getFormatedErrorMessages() << std::endl;
			...
			if (result2.first == 0) {
				std::string result_conditions = result.second.asString();
				std::string expect_conditions = "(((id = 100 and age = 20 and name = 'll') or name = '3kx') and name = 'nationalstonegroup') or id = 10";
				BEAST_EXPECT(result_conditions == expect_conditions);
			}
			else {
				BEAST_EXPECT(result2.first == 0);
		...
		{
			std::string raw_string = "[{\"$or\": [{\"$and\": [{\"$and\": [{\"$or\":[{\"id\":100},{\"age\":20},{\"name\":\"ll\"}]},{\"name\": \"3kx\"}]},{\"name\": \"nationalstonegroup\"}]},{\"id\":10}]}]";
			Json::Reader reader = Json::Reader();
			Json::Value conditions;
			if (reader.parse(raw_string, conditions) == false) {
				std::cout << "parse error. " << reader.getFormatedErrorMessages() << std::endl;
			...
			if (result2.first == 0) {
				std::string result_conditions = result.second.asString();
				std::string expect_conditions = "(((id = 100 or age = 20 or name = 'll') and name = '3kx') and name = 'nationalstonegroup') or id = 10";
				BEAST_EXPECT(result_conditions == expect_conditions);
			}
			else {
				BEAST_EXPECT(result2.first == 0);
		...
		{
			// 嵌套 5 层
			std::string raw_string = "[{\"$or\": [{\"$and\": [{\"$or\": [{\"$or\":[{\"$and\":[{\"age\":36},{\"name\":\"zx\"}]},{\"id\":100},{\"age\":20},{\"name\":\"ll\"}]},{\"name\": \"3kx\"}]},{\"name\": \"nationalstonegroup\"}]},{\"id\":10}]}]";
			Json::Reader reader = Json::Reader();
			Json::Value conditions;
			if (reader.parse(raw_string, conditions) == false) {
				std::cout << "parse error. " << reader.getFormatedErrorMessages() << std::endl;
			...
			if (result2.first == 0) {
				std::string result_conditions = result.second.asString();
				std::string expect_conditions = "((((age = 36 and name = 'zx') or id = 100 or age = 20 or name = 'll') or name = '3kx') and name = 'nationalstonegroup') or id = 10";
				BEAST_EXPECT(result_conditions == expect_conditions);
			}
			else {
				BEAST_EXPECT(result2.first == 0);
		...
		{
			// 嵌套 5 层
			std::string raw_string = "[{\"$or\": [{\"$and\": [{\"$or\": [{\"$or\":[{\"$and\":[{\"age\":36},{\"name\":\"zx\"}]},{\"id\":100},{\"age\":20},{\"name\":\"ll\"}]},{\"name\": \"3kx\"}]},{\"name\": \"nationalstonegroup\"}]},{\"$and\": [{\"$or\": [{\"$or\":[{\"$and\":[{\"age\":36},{\"name\":\"zx\"}]},{\"id\":100},{\"age\":20},{\"name\":\"ll\"}]},{\"name\": \"3kx\"}]},{\"name\": \"nationalstonegroup\"}]}]}]";
			Json::Reader reader = Json::Reader();
			Json::Value conditions;
			if (reader.parse(raw_string, conditions) == false) {
				std::cout << "parse error. " << reader.getFormatedErrorMessages() << std::endl;
			...
			if (result2.first == 0) {
				std::string result_conditions = result.second.asString();
				std::string expect_conditions = "((((age = 36 and name = 'zx') or id = 100 or age = 20 or name = 'll') or name = '3kx') and name = 'nationalstonegroup') or ((((age = 36 and name = 'zx') or id = 100 or age = 20 or name = 'll') or name = '3kx') and name = 'nationalstonegroup')";
				BEAST_EXPECT(result_conditions == expect_conditions);
			}
			else {
				BEAST_EXPECT(result2.first == 0);
	...
	void test_bugcrash() {
		...
		{
			std::string raw_string = "[{\"$and\":[{\"$or\":[]}, {\"name\":\"nationalstonegroup\"}]}]";
			Json::Reader reader = Json::Reader();
			Json::Value conditions;
			if (reader.parse(raw_string, conditions) == false) {
				std::cout << "parse error. " << reader.getFormatedErrorMessages() << std::endl;
