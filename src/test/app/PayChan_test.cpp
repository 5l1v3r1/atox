...
struct PayChan_test : public beast::unit_test::suite
{
    ...
    {
        Serializer msg;
        serializePayChanAuthorization (msg, channel, authAmt.wfn ());
    ...
    testSimple ()
    {
        ...
        env (create (alice, bob, WFN (1000), settleDelay, pk));
        auto const chan = channel (*env.current (), alice, bob);
        BEAST_EXPECT (channelBalance (*env.current (), chan) == WFN (0));
        BEAST_EXPECT (channelAmount (*env.current (), chan) == WFN (1000));

        {
            auto const preAlice = env.balance (alice);
            env (fund (alice, chan, WFN (1000)));
            auto const feeDrops = env.current ()->fees ().base;
            BEAST_EXPECT (env.balance (alice) == preAlice - WFN (1000) - feeDrops);
        }

        auto chanBal = channelBalance (*env.current (), chan);
        auto chanAmt = channelAmount (*env.current (), chan);
        BEAST_EXPECT (chanBal == WFN (0));
        BEAST_EXPECT (chanAmt == WFN (2000));

        {
            // bad amounts (non-wfn, negative amounts)
        ...
        // invalid account
        env (create (alice, "noAccount", WFN (1000), settleDelay, pk),
            ter (tecNO_DST));
        // can't create channel to the same account
        env (create (alice, alice, WFN (1000), settleDelay, pk),
             ter (temDST_IS_SRC));
        // invalid channel
        env (fund (alice, channel (*env.current (), alice, "noAccount"), WFN (1000)),
            ter (tecNO_ENTRY));
        // not enough funds
        env (create (alice, bob, WFN (10000), settleDelay, pk),
            ter (tecUNFUNDED));

        {
            // No signature claim with bad amounts (negative and non-wfn)
            auto const iou = USDA (100).value ();
            auto const negWFN = WFN (-100).value ();
            auto const posWFN = WFN (100).value ();
            env (claim (alice, chan, iou, iou), ter (temBAD_AMOUNT));
            env (claim (alice, chan, posWFN, iou), ter (temBAD_AMOUNT));
            env (claim (alice, chan, iou, posWFN), ter (temBAD_AMOUNT));
            env (claim (alice, chan, negWFN, negWFN), ter (temBAD_AMOUNT));
            env (claim (alice, chan, posWFN, negWFN), ter (temBAD_AMOUNT));
            env (claim (alice, chan, negWFN, posWFN), ter (temBAD_AMOUNT));
        }
        {
            // No signature claim more than authorized
            auto const delta = WFN (500);
            auto const reqBal = chanBal + delta;
            auto const authAmt = reqBal + WFN (-100);
        ...
        }
        {
            ...
            auto const preBob = env.balance (bob);
            STAmount const authAmt = chanBal + WFN (500);
        ...
        env (fund (bob, chan, WFN (1000)), ter (tecNO_PERMISSION));
        BEAST_EXPECT (channelBalance (*env.current (), chan) == chanBal);
        BEAST_EXPECT (channelAmount (*env.current (), chan) == chanAmt);

        {
            // Wrong signing key
            auto const sig =
                signClaimAuth (bob.pk (), bob.sk (), chan, WFN (1500));
            env (claim (bob, chan, WFN (1500).value (), WFN (1500).value (),
                     Slice (sig), bob.pk ()),
                ter (temBAD_SIGNER));
            BEAST_EXPECT (channelBalance (*env.current (), chan) == chanBal);
            BEAST_EXPECT (channelAmount (*env.current (), chan) == chanAmt);
    ...
    testCancelAfter ()
    {
        ...
        auto const carol = Account ("carol");
        {
            ...
            auto const channelFunds = WFN (1000);
            env (create (
    ...
    testExpiration ()
    {
        ...
        env (fund (alice, chan, WFN (1),
            ...
            ter (temBAD_EXPIRATION));
        BEAST_EXPECT (!channelExpiration (*env.current (), chan));
        env (fund (alice, chan, WFN (1), NetClock::time_point{minExpiration}));
        env.close (minExpiration);
        // Try to extend the expiration after the expiration has already passed
        env (fund (
            alice, chan, WFN (1), NetClock::time_point{minExpiration + 1000s}));
        BEAST_EXPECT (!channelExists (*env.current (), chan));
    }
    ...
    {
        // auth amount defaults to balance if not present
        testcase ("Disallow WFN");
        using namespace jtx;
        using namespace std::literals::chrono_literals;
        {
            // Create a channel where dst disallows WFN
            Env env (*this);
            auto const alice = Account ("alice");
            auto const bob = Account ("bob");
            env.fund (WFN (10000), alice, bob);
            env (fset (bob, asfDisallowWFN));
            auto const pk = alice.pk ();
            auto const settleDelay = 3600s;
            auto const channelFunds = WFN (1000);
            env (create (alice, bob, channelFunds, settleDelay, pk),
        ...
        {
            // Claim to a channel where dst disallows WFN
            // (channel is created before disallow wfn is set)
            Env env (*this);
            auto const alice = Account ("alice");
            auto const bob = Account ("bob");
            env.fund (WFN (10000), alice, bob);
            ...
            env (fset (bob, asfDisallowWFN));
            auto const preBob = env.balance (bob);
            auto const reqBal = WFN (500).value();
    ...
    testMalformedPK ()
    {
        ...
        auto jv = create (alice, bob, WFN (1000), settleDelay, pk);
