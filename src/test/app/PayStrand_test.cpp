...
bool
equal(std::unique_ptr<Step> const& s1, WFNEndpointStepInfo const& wfnsi)
{
    if (!s1)
        return false;
    return test::wfnEndpointStepEqual(*s1, wfnsi.acc);
}
...
ape(AccountID const& a)
{
    return STPathElement(
        STPathElement::typeAccount, a, wfnCurrency(), wfnAccount());
};
...
{
    return STPathElement(
        STPathElement::typeIssuer, wfnAccount(), wfnCurrency(), account);
};
...
{
    return STPathElement(
        STPathElement::typeCurrency, wfnAccount(), c, wfnAccount());
};
...
public:
    ...
    valid() const
    {
        return
            ...
            (!hasAny({SB::wfn, SB::existingCur, SB::prevCur}) || has(SB::cur)) &&
            // These will be duplicates
            (count({SB::wfn, SB::existingCur, SB::prevCur}) <= 1) &&
...
struct ExistingElementPool
{
        ...
        boost::optional<size_t> const& offererIndex)
    {
        ...
        // create offers to/from wfn to every other ious
        for (auto const& iou : ious)
        {
            auto const owner =
                offererIndex ? accounts[*offererIndex] : iou.account;
            env(offer(owner, iou(1000), WFN(1000)), txflags(tfPassive));
            env(offer(owner, WFN(1000), iou(1000)), txflags(tfPassive));
            env.close();
        }
    }

    std::int64_t
    totalWFN(ReadView const& v, bool incRoot)
    {
        std::uint64_t totalWFN = 0;
    ...
    // Check that the balances for all accounts for all currencies & WFN are the
    // same
    bool
    checkBalances(ReadView const& v1, ReadView const& v2)
    {
        ...
        auto wfnBalance = [](ReadView const& v, ripple::Keylet const& k) {
        ...
        {
            {
                ...
                auto const b1 = wfnBalance(v1, ak);
                auto const b2 = wfnBalance(v2, ak);
                totalWFN[0] += b1.mantissa();
                totalWFN[1] += b2.mantissa();
                if (b1 != b2)
                    diffs.emplace_back(b1, b2, wfnAccount(), *ai1);
            }
            for (auto ai2 = accounts.begin(); ai2 != aie; ++ai2)
            {
                if (ai1 >= ai2)
...
struct PayStrandAllPairs_test : public beast::unit_test::suite
{
    ...
    testAllPairs(std::initializer_list<uint256> fs)
    {
        ...
        auto callback = [&](
            ...
            // check combinations of src and dst currencies (inc wfn)
            // Check the results
            auto const terMatch = [&] {
                ...
                // wfn followed by offer that doesn't specify both currency and
                // issuer (and currency is not wfn, if specifyed)
                if (isWFN(sendMax) &&
                    !(p[0].hasCurrency() && isWFN(p[0].getCurrency())) &&
                ...
                for (size_t i = 0; i < p.size() - 1; ++i)
                {
                    ...
                    if ((tCur & STPathElement::typeCurrency) &&
                        isWFN(p[i].getCurrency()) &&
                        (tNext & STPathElement::typeAccount) &&
                        !isWFN(p[i + 1].getAccountID()))
                    {
                        return true;
                    }
                }
        ...
        for (auto const srcAmtIsWFN : {false, true})
        {
            for (auto const dstAmtIsWFN : {false, true})
            {
                for (auto const hasPrefix : {false, true})
                {
                    ...
                    STAmount const sendMax{
                        srcAmtIsWFN ? wfnIssue() : Issue{eep.getAvailCurrency(),
                                                         eep.getAvailAccount()},
                        -1,  // (-1 == no limit)
                        0};

                    STAmount const deliver{
                        dstAmtIsWFN ? wfnIssue() : Issue{eep.getAvailCurrency(),
                                                         eep.getAvailAccount()},
                        1,
                        0};
...
struct PayStrand_test : public beast::unit_test::suite
{
    ...
    testToStrand(std::initializer_list<uint256> fs)
    {
        ...
        using B = ripple::Book;
        using WFNS = WFNEndpointStepInfo;
...

        {
            ...
            {
                STPath const path = STPath({ipe(USD), cpe(wfnCurrency())});
                auto r = toStrand(
                    *env.current(),
                    alice,
                    alice,
...

        {
            ...
            // Path with WFN cross currency bridged payment
            test(
                env,
                EUR,
                USD.issue(),
                STPath({cpe(wfnCurrency())}),
                tesSUCCESS,
                D{alice, gw, usdC},
                B{USD, WFN},
                B{WFN, EUR},
            ...
            // WFN -> WFN transaction can't include a path
            test(env, WFN, boost::none, STPath({ape(carol)}), temBAD_PATH);
            ...
            test(
                ...
                STPath({STPathElement(
                    0, wfnAccount(), wfnCurrency(), wfnAccount())}),
                temBAD_PATH);
        ...
        {
            Env env(*this, with_features(fs));
            env.fund(WFN(10000), alice, bob, noripple(gw));
            env.trust(USD(1000), alice, bob);
            env(pay(gw, alice, USD(100)));
...

        {
            ...
            path.emplace_back(boost::none, wfnCurrency(), boost::none);
            ...
            auto r = toStrand(
                *env.current(),
                ...
                env.app().logs().journal("Flow"));
            BEAST_EXPECT(r.first == tesSUCCESS);
            BEAST_EXPECT(equal(r.second, D{alice, gw, usdC}, B{USD.issue(), wfnIssue()}, WFNS{bob}));
        }
    }
    ...
    testRIPD1373(std::initializer_list<uint256> fs)
    {
        ...
        if (hasFeature(fix1373, fs))
        {
            ...
            env(offer(bob, WFN(100), bob["USD"](100)), txflags(tfPassive));
            env(offer(gw, WFN(100), USD(100)), txflags(tfPassive));
...

        {
            ...
            env(offer(bob, WFN(100), USD(100)), txflags(tfPassive));
            env(offer(bob, USD(100), WFN(100)), txflags(tfPassive));

            // payment path: WFN -> WFN/USD -> USD/WFN
            env(pay(alice, carol, WFN(100)),
                path(~USD, ~WFN),
                txflags(tfNoRippleDirect),
                ter(temBAD_SEND_WFN_PATHS));
        }
    ...
    testLoop(std::initializer_list<uint256> fs)
    {
...

        {
            ...
            // payment path: USD -> USD/WFN -> WFN/USD
            env(pay(alice, carol, USD(100)),
                sendmax(USD(100)),
                path(~WFN, ~USD),
                txflags(tfNoRippleDirect),
        ...
        }
        {
            ...
            // payment path: WFN->WFN/USD->USD/EUR->USD/CNY
