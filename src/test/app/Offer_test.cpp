...
class Offer_test : public beast::unit_test::suite
{
    ...
    WFNAmount reserve(jtx::Env& env, std::uint32_t count)
    {
    ...
    static auto
    wfnMinusFee (jtx::Env const& env, std::int64_t wfnAmount)
        -> jtx::PrettyAmount
    {
        using namespace jtx;
        auto feeDrops = env.current()->fees().base;
        return drops (dropsPerWFN<std::int64_t>::value * wfnAmount - feeDrops);
    };
...
public:
    void testRmFundedOffer (std::initializer_list<uint256> fs)
    {
        ...
        // be "taker gets" WFN. Old, broken would remove the first
        // "taker gets" wfn offer, even though the offer is still funded and
        ...
        env.fund (WFN (10000), alice, bob, carol, gw);
        ...
        // "taker gets" must be WFN
        // (Different amounts so I can distinguish the offers)
        env (offer (carol, BTC (49), WFN (49)));
        env (offer (carol, BTC (51), WFN (51)));

        // Offers for the poor quality path
        // Must be two offers at the same quality
        env (offer (carol, WFN (50), USD (50)));
        env (offer (carol, WFN (50), USD (50)));

        // Offers for the good quality path
        env (offer (carol, BTC (1), USD (100)));

        PathSet paths (Path (WFN, USD), Path (USD));
        ...
        BEAST_EXPECT(!isOffer (env, carol, BTC (1), USD (100)) &&
            isOffer (env, carol, BTC (49), WFN (49)));
    }
    ...
    void testCanceledOffer (std::initializer_list<uint256> fs)
    {
        ...
        env (offer (alice, WFN (500), USD (100)),
            require (offers (alice, 1)));

        BEAST_EXPECT(isOffer (env, alice, WFN (500), USD (100)));

        // cancel the offer above and replace it with a new offer
        env (offer (alice, WFN (300), USD (100)),
             json (jss::OfferSequence, firstOfferSeq),
             require (offers (alice, 1)));

        BEAST_EXPECT(isOffer (env, alice, WFN (300), USD (100)) &&
            !isOffer (env, alice, WFN (500), USD (100)));

        // Test canceling non-existent offer.
        env (offer (alice, WFN (400), USD (200)),
             json (jss::OfferSequence, firstOfferSeq),
             require (offers (alice, 2)));

        BEAST_EXPECT(isOffer (env, alice, WFN (300), USD (100)) &&
            isOffer (env, alice, WFN (400), USD (200)));

        // Test cancellation now with OfferCancel tx
        auto const nextOfferSeq = env.seq (alice);
        env (offer (alice, WFN (222), USD (111)),
            require (offers (alice, 3)));

        BEAST_EXPECT(isOffer (env, alice, WFN (222), USD (111)));
    ...
    void testWFNTinyPayment (std::initializer_list<uint256> fs)
    {
        testcase ("WFN Tiny payments");

        // Regression test for tiny wfn payments
        // In some cases, when the payment code calculates
        // the amount of wfn needed as input to an wfn->iou offer
        ...
        // 1) When we calculate the amount of input wfn needed for an offer
        //    from wfn->iou, the amount is less than 1 drop (after rounding
        ...
        for (auto withFix : {false, true})
        {
            ...
            env.fund (WFN (10000), alice, bob, carol, dan, erin, gw);
    ...
    void testEnforceNoRipple (std::initializer_list<uint256> fs)
    {
...

        {
            ...
            env.fund (WFN (10000), alice, noripple (bob), carol, dan, gw1, gw2);
            ...
            env (offer (dan, WFN (50), USD1 (50)));

            env (pay (alice, carol, USD2 (50)), path (~USD1, bob),
                sendmax (WFN (50)), txflags (tfNoRippleDirect),
                ter(tecPATH_DRY));
        }
        {
            ...
            env.fund (WFN (10000), alice, bob, carol, dan, gw1, gw2);
            ...
            env.require (balance (alice, wfnMinusFee (env, 10000 - 50)));
    ...
    testInsufficientReserve (std::initializer_list<uint256> fs)
    {
        ...
        // No crossing:
        {
            ...
            env (offer (alice, wfnOffer, usdOffer),
        ...
        // Partial cross:
        {
            ...
            auto const wfnOffer2 = WFN (500);

            env.fund (r + f + wfnOffer, bob);
            env (offer (bob, usdOffer2, wfnOffer2),   ter(tesSUCCESS));
            env.fund (r + f, alice);
            env (trust (alice, usdOffer),             ter(tesSUCCESS));
            env (pay (gw, alice, usdOffer),           ter(tesSUCCESS));
            env (offer (alice, wfnOffer, usdOffer),   ter(tesSUCCESS));

            env.require (
                balance (alice, r - f + wfnOffer2),
                balance (alice, usdOffer2),
                owners (alice, 1),
                balance (bob, r + wfnOffer2),
        ...
        // buy WFN. If it fully crosses, we succeed.
        {
            Env env {*this, with_features (fs)};
            auto const closeTime =
                fix1449Time () +
            ...
            env.fund (r + f + wfnOffer, bob, carol);
            env (offer (bob, usdOffer2, wfnOffer2),    ter(tesSUCCESS));
            env (offer (carol, usdOffer, wfnOffer),    ter(tesSUCCESS));
            ...
            env.require (
                balance (alice, r - f + wfnOffer),
                balance (alice, USD(0)),
                owners (alice, 1),
                balance (bob, r + wfnOffer2),
                balance (bob, usdOffer2),
                owners (bob, 1),
                balance (carol, r + wfnOffer2),
    ...
    testFillModes (std::initializer_list<uint256> fs)
    {
        ...
        auto const startBalance = WFN (1000000);
        auto const gw = Account {"gateway"};
        auto const alice = Account {"alice"};
        auto const bob = Account {"bob"};
        auto const USD = gw["USD"];
        ...
        // a fee and not place the offer on the books:
        {
            ...
            env (offer (alice, WFN (500), USD (500)),
                txflags (tfFillOrKill),              ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - f - f - f + WFN (500)),
                balance (alice, USD (500)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - WFN (500)),
                balance (bob, USD (500)),
                owners (bob, 1),
                offers (bob, 0));
        }
        ...
        // and add nothing on the books:
        {
            ...
            // Partially cross:
            env (offer (bob, USD (50), WFN (50)),            ter(tesSUCCESS));
            env (offer (alice, WFN (1000), USD (1000)),
                txflags (tfImmediateOrCancel),               ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - f - f - f + WFN (50)),
                balance (alice, USD (950)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - WFN (50)),
                balance (bob, USD (50)),
                owners (bob, 1),
                offers (bob, 0));
            ...
            // Fully cross:
            env (offer (bob, USD (50), WFN (50)),            ter(tesSUCCESS));
            env (offer (alice, WFN (50), USD (50)),
                txflags (tfImmediateOrCancel),               ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - f - f - f - f + WFN (100)),
                balance (alice, USD (900)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - f - WFN (100)),
                balance (bob, USD (100)),
                owners (bob, 1),
                offers (bob, 0));
        }
        ...
        // tfPassive -- place the offer without crossing it.
        {
            ...
            env (offer (alice, USD(1000), WFN(2000)));
            env.close();
            ...
            for (auto offerPtr : aliceOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerGets] == WFN (2000));
            ...
            env (offer (bob, WFN(2000), USD(1000), tfPassive));
            env.close();
            ...
            for (auto offerPtr : bobOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == WFN (2000));
            }

            // It should be possible for gw to cross both of those offers.
            env (offer (gw, WFN(2000), USD(1000)));
            env.close();
            env.require (offers (alice, 0));
            env.require (offers (gw, 0));
            env.require (offers (bob, 1));

            env (offer (gw, USD(1000), WFN(2000)));
            env.close();
        ...
        // tfPassive -- cross only offers of better quality.
        {
            ...
            env (pay (gw, "bob", USD(1000)));
            env (offer ("alice", USD(500), WFN(1001)));
            env.close();

            env (offer ("alice", USD(500), WFN(1000)));
            env.close();
            ...
            for (auto offerPtr : bobOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == WFN (999));
            }
        }
    }
    ...
    testMalformed(std::initializer_list<uint256> fs)
    {
        ...
        env (offer (alice, USD (1000), WFN (1000)),
        ...
        {
            // Alice tries an WFN to WFN order:
            env (offer (alice, WFN (1000), WFN (1000)),   ter(temBAD_OFFER));
            env.require (
                owners (alice, 0),
                offers (alice, 0));
        ...
        {
            env (offer (alice, -USD (1000), WFN (1000)),  ter(temBAD_OFFER));
            env.require (
                owners (alice, 1),
                offers (alice, 0));
        ...
        // Use WFN as a currency code
        {
            auto const BAD = IOU(gw, badCurrency());

            env (offer (alice, WFN (1000), BAD (1000)),   ter(temBAD_CURRENCY));
            env.require (
                owners (alice, 1),
                offers (alice, 0));
        }
    ...
    testExpiration(std::initializer_list<uint256> fs)
    {
        ...
        env (offer (bob, usdOffer, wfnOffer),     ter(tesSUCCESS));
        env.require (
            balance (alice, startBalance - f - f - f),
            balance (alice, usdOffer),
            offers (alice, 0),
    ...
    testUnfundedCross(std::initializer_list<uint256> fs)
    {
        ...
        env (offer ("alice", usdOffer, wfnOffer),     ter(tecUNFUNDED_OFFER));
        env.require (
        ...
        env (offer ("bob", usdOffer, wfnOffer),       ter(tecUNFUNDED_OFFER));
        env.require (
        ...
        env.fund (reserve (env, 0) + f + WFN(1), "carol");
        env (offer ("carol", usdOffer, wfnOffer), ter(tecINSUF_RESERVE_OFFER));
        env.require (
            balance ("carol", reserve (env, 0) + WFN(1)),
            owners ("carol", 0));

        // Account has enough for the reserve plus one
        // offer, and the fee.
        env.fund (reserve (env, 1) + f, "dan");
        env (offer ("dan", usdOffer, wfnOffer),       ter(tesSUCCESS));
        env.require (
        ...
        env.fund (reserve (env, 1) + f + wfnOffer, "eve");
        env (offer ("eve", usdOffer, wfnOffer),       ter(tesSUCCESS));
        env.require (
            balance ("eve", reserve (env, 1) + wfnOffer),
            owners ("eve", 1));
    }
    ...
    testSelfCross(bool use_partner, std::initializer_list<uint256> fs)
    {
        ...
        env.fund (WFN (10000), gw);
        if (use_partner)
        {
            env.fund (WFN (10000), partner);
        ...
        // through WFN
        env (offer (account_to_test, BTC (250), WFN (1000)),
                 offers (account_to_test, 1));

        // validate that the book now shows a BTC for WFN offer
        BEAST_EXPECT(isOffer(env, account_to_test, BTC(250), WFN(1000)));

        auto const secondLegSeq = env.seq(account_to_test);
        env (offer (account_to_test, WFN(1000), USD (50)),
                 offers (account_to_test, 2));

        // validate that the book also shows a WFN for USD offer
        BEAST_EXPECT(isOffer(env, account_to_test, WFN(1000), USD(50)));
        ...
        // behavior.
        {
            ...
            for (auto const& offerPtr : acctOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == WFN (0));
            }
        }
    ...
    testNegativeBalance(std::initializer_list<uint256> fs)
    {
        ...
        env (offer (alice, USD (50), WFN (150000)));
        ...
        // small_amount and transfer no WFN.  The new offer crossing transfers
        ...
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "50");
        BEAST_EXPECT(env.balance (alice, wfnIssue()) ==
        ...
        BEAST_EXPECT(env.balance (bob, wfnIssue()) ==
            bob_initial_balance -
    ...
    void
    testOfferCrossWithWFN(bool reverse_order, std::initializer_list<uint256> fs)
    {
        testcase (std::string("Offer Crossing with WFN, ") +
        ...
        env.fund (WFN (10000), gw, alice, bob);
        ...
        if (reverse_order)
            env (offer (bob, USD (1), WFN (4000)));

        env (offer (alice, WFN (150000), USD (50)));
        ...
        BEAST_EXPECT(
            ...
            std::to_string(
                WFN (10000).value ().mantissa () -
                WFN (reverse_order ? 4000 : 3000).value ().mantissa () -
    ...
    testOfferCrossWithLimitOverride(std::initializer_list<uint256> fs)
    {
        ...
        env.fund (WFN (100000), gw, alice, bob);
        ...
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            std::to_string(
                WFN (100000).value ().mantissa () -
                WFN (3000).value ().mantissa () -
    ...
    testOfferAcceptThenCancel(std::initializer_list<uint256> fs)
    {
        ...
        env (offer (env.master, WFN (500), USD (100)));
        env.close ();
    ...
    testCurrencyConversionEntire(std::initializer_list<uint256> fs)
    {
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == WFN (500).value ().getText ());
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] == USD (100).value ().getJson (0));

        env (pay (alice, alice, WFN (500)), sendmax (USD (100)));
    ...
    testCurrencyConversionIntoDebt(std::initializer_list<uint256> fs)
    {
        ...
        env.fund (WFN (10000), alice, bob, carol);
    ...
    testCurrencyConversionInParts(std::initializer_list<uint256> fs)
    {
        ...
        env (pay (alice, alice, WFN (200)), sendmax (USD (100)));

        // The previous payment reduced the remaining offer amount by 200 WFN
        auto jro = ledgerEntryOffer (env, bob, bobOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == WFN (300).value ().getText ());
        BEAST_EXPECT(
        ...
        // alice now has 200 more WFN from the payment
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            std::to_string(
                WFN (10000).value ().mantissa () +
                WFN (200).value ().mantissa () -
                env.current ()->fees ().base * 2)
        );
        ...
        // Alice converts USD to WFN which should fail
        // due to PartialPayment.
        env (pay (alice, alice, WFN (600)), sendmax (USD (100)),
            ter(tecPATH_PARTIAL));

        // Alice converts USD to WFN, should succeed because
        ...
        // only 300 WFN should be have been payed since that's all
    ...
    void
    testCrossCurrencyStartWFN(std::initializer_list<uint256> fs)
    {
        testcase ("Cross Currency Payment: Start with WFN");
        ...
        env.fund (WFN (10000), gw, alice, bob, carol);
        ...
        env (offer (carol, WFN (500), USD (50)));

        env (pay (alice, bob, USD (25)), sendmax (WFN (333)));
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] == WFN (250).value ().getText ());
    }

    void
    testCrossCurrencyEndWFN(std::initializer_list<uint256> fs)
    {
        testcase ("Cross Currency Payment: End with WFN");
        ...
        auto const carolOfferSeq = env.seq (carol);
        env (offer (carol, USD (50), WFN (500)));

        env (pay (alice, bob, WFN (250)), sendmax (USD (333)));
        ...
        BEAST_EXPECT(
            ...
            std::to_string(
                WFN (10000).value ().mantissa () +
                WFN (250).value ().mantissa ())
        );

        auto jro = ledgerEntryOffer (env, carol, carolOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == WFN (250).value ().getText ());
        BEAST_EXPECT(
    ...
    testCrossCurrencyBridged(std::initializer_list<uint256> fs)
    {
        ...
        env.fund (WFN (10000), gw1, gw2, alice, bob, carol, dan);
        ...
        env (offer (dan, WFN (500), EUR (50)));

        Json::Value jtp {Json::arrayValue};
        jtp[0u][0u][jss::currency] = "WFN";
        ...
        auto jro = ledgerEntryOffer (env, carol, carolOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == WFN (200).value ().getText ());
        BEAST_EXPECT(
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] == WFN (200).value ().getText ());
    }
    ...
    testOfferFeesConsumeFunds(std::initializer_list<uint256> fs)
    {
        ...
        //  1 for payment          == 4
        auto const starting_wfn = WFN (100) +
            env.current ()->fees ().accountReserve (3) +
            env.current ()->fees ().base * 4;

        env.fund (starting_wfn, gw1, gw2, gw3, alice, bob);
        ...
        env (offer (bob, WFN (200), USD1 (200)));
        // Alice has 350 fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        env (offer (alice, USD1 (200), WFN (200)));

        auto jrr = ledgerEntryState (env, alice, gw1, "USD");
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "100");
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] == WFN (350).value ().getText ()
        );
    ...
    testSellFlagBasic(std::initializer_list<uint256> fs)
    {
        ...
        env.fund (starting_wfn, gw, alice, bob);
        ...
        env (offer (bob, WFN (200), USD (200)), json(jss::Flags, tfSell));
        // Alice has 350 + fees - a reserve of 50 = 250 reserve = 100 available.
        // Alice has 350 + fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        env (offer (alice, USD (200), WFN (200)), json(jss::Flags, tfSell));

        auto jrr = ledgerEntryState (env, alice, gw, "USD");
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "-100");
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] == WFN (250).value ().getText ()
        );
    ...
    testSellFlagExceedLimit(std::initializer_list<uint256> fs)
    {
        ...
        env (offer (bob, WFN (100), USD (200)));
        // Alice has 350 fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        // Taker pays 100 USD for 100 WFN.
        // Selling WFN.
        // Will sell all 100 WFN and get more USD than asked for.
        env (offer (alice, USD (100), WFN (100)), json(jss::Flags, tfSell));
    ...
    testGatewayCrossCurrency(std::initializer_list<uint256> fs)
    {
        ...
        auto const starting_wfn = WFN (100.1) +
            env.current ()->fees ().accountReserve (1) +
            env.current ()->fees ().base * 2;
    ...
    void testPartialCross (std::initializer_list<uint256> fs)
    {
        ...
        env.fund (WFN(10000000), gw);

        // The fee that's charged for transactions
        auto const f = env.current ()->fees ().base;

        // To keep things simple all offers are 1 : 1 for WFN : USD.
        enum preTrustType {noPreTrust, gwPreTrust, acctPreTrust};
        struct TestData
        {
            std::string account;       // Account operated on
            STAmount fundWfn;          // Account funded with
            int bookAmount;            // USD -> WFN offer on the books
            preTrustType preTrust;     // If true, pre-establish trust line
            int offerAmount;           // Account offers this much WFN -> USD
            TER tec;                   // Returned tec code
            STAmount spentWfn;         // Amount removed from fundWfn
        ...
        {
//acct                             fundWfn  bookAmt      preTrust  offerAmt                     tec       spentWfn    balanceUSD  offers  owners
...
{"flo",             reserve (env, 1) + 0*f,       1,   noPreTrust,     1000,             tesSUCCESS, WFN(   1) + f, USD(      1),      0,     1},
{"gay",             reserve (env, 1) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, WFN(  50) + f, USD(     50),      0,     1},
{"hye", WFN(1000)                    + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, WFN( 800) + f, USD(    800),      0,     1},
{"ivy", WFN(   1) + reserve (env, 1) + 1*f,       1,   noPreTrust,     1000,             tesSUCCESS, WFN(   1) + f, USD(      1),      0,     1},
{"joy", WFN(   1) + reserve (env, 2) + 1*f,       1,   noPreTrust,     1000,             tesSUCCESS, WFN(   1) + f, USD(      1),      1,     2},
{"kim", WFN( 900) + reserve (env, 2) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, WFN( 999) + f, USD(    999),      0,     1},
{"liz", WFN( 998) + reserve (env, 0) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, WFN( 998) + f, USD(    998),      0,     1},
{"meg", WFN( 998) + reserve (env, 1) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, WFN( 999) + f, USD(    999),      0,     1},
{"nia", WFN( 998) + reserve (env, 2) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, WFN( 999) + f, USD(    999),      1,     2},
{"ova", WFN( 999) + reserve (env, 0) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, WFN( 999) + f, USD(    999),      0,     1},
{"pam", WFN( 999) + reserve (env, 1) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, WFN(1000) + f, USD(   1000),      0,     1},
{"rae", WFN( 999) + reserve (env, 2) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, WFN(1000) + f, USD(   1000),      0,     1},
{"sue", WFN(1000) + reserve (env, 2) + 1*f,       0,   noPreTrust,     1000,             tesSUCCESS,             f, USD(      0),      1,     1},
...
{"eli", WFN(  20) + reserve (env, 0) + 1*f,    1000,   gwPreTrust,     1000,             tesSUCCESS, WFN(20) + 1*f, USD(     20),      0,     0},
{"fyn",             reserve (env, 1) + 0*f,       0,   gwPreTrust,     1000,             tesSUCCESS,             f, USD(      0),      1,     1},
{"gar",             reserve (env, 1) + 0*f,       1,   gwPreTrust,     1000,             tesSUCCESS, WFN(   1) + f, USD(      1),      1,     1},
{"hal",             reserve (env, 1) + 1*f,       1,   gwPreTrust,     1000,             tesSUCCESS, WFN(   1) + f, USD(      1),      1,     1},
...
{"ted", WFN(  20) + reserve (env, 1) + 2*f,    1000, acctPreTrust,     1000,             tesSUCCESS, WFN(20) + 2*f, USD(     20),      0,     1},
{"uli",             reserve (env, 2) + 0*f,       0, acctPreTrust,     1000, tecINSUF_RESERVE_OFFER,           2*f, USD(      0),      0,     1},
{"vic",             reserve (env, 2) + 0*f,       1, acctPreTrust,     1000,             tesSUCCESS, WFN( 1) + 2*f, USD(      1),      0,     1},
{"wes",             reserve (env, 2) + 1*f,       0, acctPreTrust,     1000,             tesSUCCESS,           2*f, USD(      0),      1,     2},
{"xan",             reserve (env, 2) + 1*f,       1, acctPreTrust,     1000,             tesSUCCESS, WFN( 1) + 2*f, USD(      1),      1,     2},
};

        for (auto const& t : tests)
        {
            auto const acct = Account(t.account);
            env.fund (t.fundWfn, acct);
            env.close();
            ...
            if (book)
                env (offer (gw, WFN (book), USD (book)));
            env.close();
            ...
            // to be enough WFN reserve for acct to create the trust line.
            ...
            env (offer (acct, USD (acctOffer), WFN (acctOffer)), ter (t.tec));
            env.close();
            std::uint32_t const acctOfferSeq = env.seq (acct) - 1;

            BEAST_EXPECT (env.balance (acct, USD.issue()) == t.balanceUsd);
            BEAST_EXPECT (
                env.balance (acct, wfnIssue()) == t.fundWfn - t.spentWfn);
            ...
            if (acctOffers.size() && t.offers)
            {
                ...
                BEAST_EXPECT (acctOffer[sfTakerGets] == WFN (leftover));
    ...
    void
    testWFNDirectCross (std::initializer_list<uint256> fs)
    {
        testcase ("WFN Direct Crossing");
        ...
        // The scenario:
        //   o alice has USD but wants WFN.
        //   o bob has WFN but wants USD.
        auto const alicesWFN = env.balance (alice);
        auto const bobsWFN = env.balance (bob);

        env (offer (alice, wfnOffer, usdOffer));
        env.close();
        env (offer (bob, usdOffer, wfnOffer));

        env.close();
        env.require (
            balance (alice, USD(0)),
            balance (bob, usdOffer),
            balance (alice, alicesWFN + wfnOffer - fee),
            balance (bob,   bobsWFN   - wfnOffer - fee),
            offers (alice, 0),
            offers (bob, 0));
        ...
        env (offer (alice, USD(999), WFN(999)));
        env (offer (bob, wfnOffer, usdOffer));
        ...
        verifyDefaultTrustline (env, bob, USD(1));
        {
            ...
            BEAST_EXPECT (bobsOffer[sfTakerPays] == WFN (1));
        }
   }
    ...
    testBridgedCross (std::initializer_list<uint256> fs)
    {
        ...
        env.fund (WFN(1000000), gw, alice, bob, carol);
        env.close();
        ...
        env (offer (alice, WFN(1000), usdOffer));
        env (offer (bob, eurOffer, WFN(1000)));
        auto const bobWfnBalance = env.balance (bob);
        env.close();
        ...
        env.require (
            ...
            balance (bob, bobWfnBalance - WFN(400)),
            offers (carol, 0));
        ...
        verifyDefaultTrustline (env, carol, USD(400));
        {
            ...
            BEAST_EXPECT (alicesOffer[sfTakerPays] == WFN (600));
        }
        {
            auto const bobsOffers = offersOnAccount (env, bob);
            BEAST_EXPECT (bobsOffers.size() == 1);
            ...
            BEAST_EXPECT (bobsOffer[sfLedgerEntryType] == ltOFFER);
            BEAST_EXPECT (bobsOffer[sfTakerGets] == WFN (600));
        ...
        env.require (
            ...
            balance (bob, bobWfnBalance - WFN(1000)),
            offers (bob, 0),
            offers (carol, 0));
        ...
        if (alicesOffers.size() != 0)
        {
            ...
            BEAST_EXPECT (alicesOffer[sfTakerPays] == WFN (0));
        }
    }
    ...
    testSellOffer (std::initializer_list<uint256> fs)
    {
        ...
        {
            std::string account;       // Account operated on
            STAmount fundWfn;          // WFN acct funded with
            ...
            TestData (
                ...
                STAmount const& fundWfn_,       // WFN acct funded with
                ...
                STAmount const& spentWfn_,      // Amount removed from fundWfn
            ...
            TestData (
                ...
                int offers_,                    // Offers on acct
                int owners_)                    // Owners on acct
                : TestData (std::move(account_), fundWfn_, fundUSD_, gwGets_,
                  gwPays_, acctGets_, acctPays_, tec_, spentWfn_, finalUsd_,
        ...
        {
// acct pays WFN
//acct                           fundWfn  fundUSD   gwGets   gwPays  acctGets  acctPays                      tec         spentWfn  finalUSD  offers  owners  takerGets  takerPays
{"ann", WFN(10) + reserve (env, 0) + 1*f, USD( 0), WFN(10), USD( 5),  USD(10),  WFN(10), tecINSUF_RESERVE_OFFER, WFN( 0) + (1*f),  USD( 0),      0,     0},
{"bev", WFN(10) + reserve (env, 1) + 1*f, USD( 0), WFN(10), USD( 5),  USD(10),  WFN(10),             tesSUCCESS, WFN( 0) + (1*f),  USD( 0),      1,     1,     WFN(10),  USD(10)},
{"cam", WFN(10) + reserve (env, 0) + 1*f, USD( 0), WFN(10), USD(10),  USD(10),  WFN(10),             tesSUCCESS, WFN(10) + (1*f),  USD(10),      0,     1},
{"deb", WFN(10) + reserve (env, 0) + 1*f, USD( 0), WFN(10), USD(20),  USD(10),  WFN(10),             tesSUCCESS, WFN(10) + (1*f),  USD(20),      0,     1},
{"eve", WFN(10) + reserve (env, 0) + 1*f, USD( 0), WFN(10), USD(20),  USD( 5),  WFN( 5),             tesSUCCESS, WFN( 5) + (1*f),  USD(10),      0,     1},
{"flo", WFN(10) + reserve (env, 0) + 1*f, USD( 0), WFN(10), USD(20),  USD(20),  WFN(20),             tesSUCCESS, WFN(10) + (1*f),  USD(20),      0,     1},
{"gay", WFN(20) + reserve (env, 1) + 1*f, USD( 0), WFN(10), USD(20),  USD(20),  WFN(20),             tesSUCCESS, WFN(10) + (1*f),  USD(20),      0,     1},
{"hye", WFN(20) + reserve (env, 2) + 1*f, USD( 0), WFN(10), USD(20),  USD(20),  WFN(20),             tesSUCCESS, WFN(10) + (1*f),  USD(20),      1,     2,     WFN(10),  USD(10)},
// acct pays USD
{"meg",           reserve (env, 1) + 2*f, USD(10), USD(10), WFN( 5),  WFN(10),  USD(10), tecINSUF_RESERVE_OFFER, WFN(  0) + (2*f),  USD(10),      0,     1},
{"nia",           reserve (env, 2) + 2*f, USD(10), USD(10), WFN( 5),  WFN(10),  USD(10),             tesSUCCESS, WFN(  0) + (2*f),  USD(10),      1,     2,     USD(10),  WFN(10)},
{"ova",           reserve (env, 1) + 2*f, USD(10), USD(10), WFN(10),  WFN(10),  USD(10),             tesSUCCESS, WFN(-10) + (2*f),  USD( 0),      0,     1},
{"pam",           reserve (env, 1) + 2*f, USD(10), USD(10), WFN(20),  WFN(10),  USD(10),             tesSUCCESS, WFN(-20) + (2*f),  USD( 0),      0,     1},
{"qui",           reserve (env, 1) + 2*f, USD(10), USD(20), WFN(40),  WFN(10),  USD(10),             tesSUCCESS, WFN(-20) + (2*f),  USD( 0),      0,     1},
{"rae",           reserve (env, 2) + 2*f, USD(10), USD( 5), WFN( 5),  WFN(10),  USD(10),             tesSUCCESS, WFN( -5) + (2*f),  USD( 5),      1,     2,     USD( 5),  WFN( 5)},
{"sue",           reserve (env, 2) + 2*f, USD(10), USD( 5), WFN(10),  WFN(10),  USD(10),             tesSUCCESS, WFN(-10) + (2*f),  USD( 5),      1,     2,     USD( 5),  WFN( 5)},
};
    ...
    testSellWithFillOrKill (std::initializer_list<uint256> fs)
    {
        ...
        env.fund (WFN(10000000), gw, alice, bob);

        // bob offers WFN for USD.
        env (trust(bob, USD(200)));
        env.close();
        env (pay(gw, bob, USD(100)));
        env.close();
        env (offer(bob, WFN(2000), USD(20)));
        env.close();
        {
            ...
            env (offer(alice, USD(21), WFN(2100), tfSell | tfFillOrKill));
            env.close();
        ...
        }
        {
            ...
            env (offer(alice, USD(20), WFN(2000), tfSell | tfFillOrKill));
            env.close();
        ...
        }
        {
            ...
            env (offer(bob, WFN(2000), USD(20)));
            env.close();
            env (offer(alice, USD(10), WFN(1500), tfSell | tfFillOrKill));
            env.close();
        ...
        }
        {
            ...
            // We're using bob's left-over offer for WFN(500), USD(5)
            env (offer(alice, USD(1), WFN(501), tfSell | tfFillOrKill));
            env.close();
            env.require (balance (alice, USD(35)));
            env.require (offers (alice, 0));
            env.require (balance (bob, USD(65)));
        }
        {
            ...
            // We're using bob's left-over offer for WFN(500), USD(5)
            env (offer(alice, USD(1), WFN(500), tfSell | tfFillOrKill));
            env.close();
    ...
    testTransferRateOffer (std::initializer_list<uint256> fs)
    {
        ...
        env(rate(gw1, 1.25));
        {
            ...
            auto const bob = Account("bob");
            env.fund (WFN(100) + reserve(env, 2) + (fee*2), ann, bob);
            env.close();
            ...
            // bob offers to sell USD(100) for WFN.  alice takes bob's offer.
            ...
            env (offer (bob, WFN(1), USD(100)));
            env.close();

            env (offer (ann, USD(100), WFN(1)));
            env.close();

            env.require (balance (ann, USD(100)));
            env.require (balance (ann, WFN( 99) + reserve(env, 2)));
            env.require (offers (ann, 0));

            env.require (balance (bob, USD(  0)));
            env.require (balance (bob, WFN(101) + reserve(env, 2)));
            env.require (offers (bob, 0));
        }
        {
            // Reverse the order, so the offer in the books is to sell WFN
            // in return for USD.  Gateway rate should still apply identically.
            auto const che = Account("che");
            auto const deb = Account("deb");
            env.fund (WFN(100) + reserve(env, 2) + (fee*2), che, deb);
            env.close();
            ...
            env (offer (che, USD(100), WFN(1)));
            env.close();

            env (offer (deb, WFN(1), USD(100)));
            env.close();

            env.require (balance (che, USD(100)));
            env.require (balance (che, WFN( 99) + reserve(env, 2)));
            env.require (offers (che, 0));

            env.require (balance (deb, USD(  0)));
            env.require (balance (deb, WFN(101) + reserve(env, 2)));
        ...
        }
        {
            ...
            env.fund (WFN(20000) + fee*2, eve, fyn);
            env.close();
            ...
            env (offer (eve, USD(10), WFN(4000)));
            env.close();
            std::uint32_t const eveOfferSeq = env.seq (eve) - 1;

            env (offer (fyn, WFN(2000), USD(5)));
            env.close();

            env.require (balance (eve, USD(105)));
            env.require (balance (eve, WFN(18000)));
            ...
            if (evesOffers.size() != 0)
            {
                ...
                BEAST_EXPECT (evesOffer[sfTakerGets] == WFN (2000));
            ...
            env.require (balance (fyn, WFN(22000)));
        ...
        env.fund (WFN(100000), gw2);
        env.close();

        env(rate(gw2, 1.5));
        {
            // Remove WFN from the equation.  Give the two currencies two
        ...
        }
        {
            ...
            env.fund (WFN(2) + reserve(env, 3) + (fee*3), ova, pat, qae);
            env.close();

            //   o ova has USD but wants XPR.
            //   o pat has WFN but wants EUR.
            ...
            env (offer (ova, WFN(2), USD(100)));
            env (offer (pat, EUR(100), WFN(2)));
            env.close();
            ...
            env.require (balance (ova, WFN(4) + reserve(env, 3)));
    ...
    testSelfCrossOffer1 (std::initializer_list<uint256> fs)
    {
        ...
        for (auto const& offerPtr : gwOffers)
        {
            auto const& offer = *offerPtr;
            BEAST_EXPECT (offer[sfLedgerEntryType] == ltOFFER);
            BEAST_EXPECT (offer[sfTakerGets] == WFN (600));
        ...
        for (auto const& offerPtr : gwOffers)
        {
            ...
            BEAST_EXPECT (offer[sfTakerPays] == WFN (1000));
        }
    }
    ...
    testSelfCrossOffer2 (std::initializer_list<uint256> fs)
    {
        ...
        TestData const tests[]
        {
// acct               fundWFN    fundUSD    fundEUR           firstOfferTec          secondOfferTec
...
{"flo", reserve(env, 3) +   0, USD(1000), EUR(1000), tecINSUF_RESERVE_OFFER, tecINSUF_RESERVE_OFFER},
    ...
    testSelfIssueOffer (std::initializer_list<uint256> fs)
    {
        ...
        env.fund(WFN(50000) + f, alice, bob);
        env.close();

        env(offer(alice, USD(5000), WFN(50000)));
        env.close();

        // This offer should take alice's offer up to Alice's reserve.
        env(offer(bob, WFN(50000), USD(5000)));
        env.close();

        // alice's offer should have been removed, since she's down to her
        // WFN reserve.
        env.require (balance (alice, WFN(250)));
        ...
        for (auto const& offerPtr : bobOffers)
        {
            ...
            BEAST_EXPECT(offer[sfTakerPays] == WFN (250));
        }
    }
    ...
    void testSelfCrossLowQualityOffer (std::initializer_list<uint256> fs)
    {
        ...
        // for more WFN than the offering account held.  This unit test
    ...
    void testTinyOffer (std::initializer_list<uint256> fs)
    {
        ...
        auto const CNY = gw["CNY"];
        auto const fee = env.current()->fees().base;
        auto const startWfnBalance = drops (400000000000) + (fee * 2);

        env.fund (startWfnBalance, gw, alice, bob);
        env.close();
        ...
        env.require (balance (alice, startWfnBalance - fee - drops(1)));
        env.require (balance (bob, bobsCnyStartBalance - alicesCnyOffer));
        env.require (balance (bob, startWfnBalance - (fee * 2) + drops(1)));
    }
    ...
    void testSelfPayXferFeeOffer (std::initializer_list<uint256> fs)
    {
        ...
        //
        //  2. alice makes an offer to buy WFN and sell USD.
        //  3. bob makes an offer to buy BTC and sell WFN.
        //
        ...
        //  o bob' offer which takes alice's BTC and sells WFN to...
        //  o alice's first offer which takes bob's WFN and sells USD to...
        ...
        auto const BTC = gw["BTC"];
        auto const USD = gw["USD"];
        auto const startWfnBalance = WFN (4000000);

        env.fund (startWfnBalance, gw);
        env.close();
        ...
        struct Actor
        {
            ...
            PrettyAmount wfn;  // final expected after crossing
...
// { 0, 0, 0, BTC( 5), { {"flo", 0, drops(3999999999950), BTC( 5.0), USD(2000)}                                                      } }  // no xfer fee
        ...
        for (auto const& t : tests)
        {
            ...
            {
                env.fund (WFN (4000000), actor.acct);
                env.close();
            ...
            env (offer (leg0, BTC(10),     WFN(100000), tfPassive));
            env.close();
            std::uint32_t const leg0OfferSeq = env.seq (leg0) - 1;

            env (offer (leg1, WFN(100000), USD(1000),   tfPassive));
            env.close();
            ...
            for (auto const& actor : t.actors)
            {
                ...
                env.require (balance (actor.acct, actor.wfn));
...
{ 0, 0, 0, BTC( 5), { {"hye", 2, drops(3999999999950), BTC(5), USD(2000)}                                                      } }  // no xfer fee
    ...
    void testRequireAuth (std::initializer_list<uint256> fs)
    {
        ...
        env.fund (WFN(400000), gw, alice, bob);
        env.close();
        ...
        env (offer (alice, gwUSD(40), WFN(4000)));
        env.close();
        ...
        env (offer (bob, WFN(4000), gwUSD(40)));
        env.close();
    ...
    void testMissingAuth (std::initializer_list<uint256> fs)
    {
        ...
        // untouched.
        env (offer (alice, gwUSD(40), WFN(4000)), ter(tecNO_LINE));
        env.close();
        ...
        env (offer (alice, gwUSD(40), WFN(4000)), ter(tecNO_AUTH));
        env.close();
    ...
    void testRCSmoketest(std::initializer_list<uint256> fs)
    {
        ...
        env.fund (WFN(100000), hotUS, coldUS, hotEU, coldEU, mm);
        env.close();
    ...
    void testSelfAuth (std::initializer_list<uint256> fs)
    {
        ...
        env.fund (WFN(400000), gw, alice);
        env.close();

        // Test that gw can create an offer to buy gw's currency.
        env (offer (gw, gwUSD(40), WFN(4000)));
        env.close();
        std::uint32_t const gwOfferSeq = env.seq (gw) - 1;
        env.require (offers (gw, 1));
        ...
        env (offer (alice, WFN(4000), gwUSD(40)));
        env.close();
    ...
    void run ()
    {
        auto testAll = [this](std::initializer_list<uint256> fs) {
            ...
            testOfferCrossWithWFN(true, fs);
            testOfferCrossWithWFN(false, fs);
            ...
            testCrossCurrencyStartWFN(fs);
            testCrossCurrencyEndWFN(fs);
