...
public:
    ...
    test (std::size_t n)
    {
        ...
        env.fund(WFN(billion), gw, "alice", "bob", "carol");
...
public:
    ...
    oversize(std::size_t n)
    {
        ...
        createOffers(env, USD, n);
        env(pay("alice", gw, USD(billion)));
        env(offer("alice", USD(1), WFN(1)), ter(std::ignore));
