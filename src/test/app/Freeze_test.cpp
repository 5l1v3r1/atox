...
class Freeze_test : public beast::unit_test::suite
{
    ...
    void testRippleState(std::initializer_list<uint256> fs)
    {
        ...
        env.fund(WFN(1000), G1, alice, bob);
        env.close();
        ...
        env(offer(alice, WFN(500), G1["USD"](100)));
        env.close();
    ...
    testGlobalFreeze(std::initializer_list<uint256> fs)
    {
        ...
        env.fund(WFN(12000), G1);
        env.fund(WFN(1000), A1);
        env.fund(WFN(20000), A2, A3, A4);
        env.close();
        ...
        env(offer(G1, WFN(10000), G1["USD"](100)), txflags(tfPassive));
        env(offer(G1, G1["USD"](100), WFN(10000)), txflags(tfPassive));
        env(offer(A1, WFN(10000), G1["USD"](100)), txflags(tfPassive));
        env(offer(A2, G1["USD"](100), WFN(10000)), txflags(tfPassive));
        env.close();
...

        {
            ...
            env(offer(A3, G1["BTC"](1), WFN(1)));

            //    test: assets can be sold on the market
            env(offer(A4, WFN(1), G1["BTC"](1)));
...

        {
            ...
            env(offer(A3, G1["BTC"](1), WFN(1)), ter(tecFROZEN));

            //    test: assets can't be sold on the market
            env(offer(A4, WFN(1), G1["BTC"](1)), ter(tecFROZEN));
        }
    ...
    testOffersWhenFrozen(std::initializer_list<uint256> fs)
    {
        ...
        env.fund(WFN(1000), G1, A3, A4);
        env.fund(WFN(2000), A2);
        env.close();
        ...
        env(offer(A3, WFN(1000), G1["USD"](1000)), txflags(tfPassive));
        env.close();

        // removal after successful payment
        //    test: make a payment with partially consuming offer
        env(pay(A2, G1, G1["USD"](1)), paths(G1["USD"]), sendmax(WFN(1)));
        env.close();
        ...
        env(offer(A4, WFN(999), G1["USD"](999)));
        env.close();
        ...
        env(offer(A2, G1["USD"](999), WFN(999)));
        affected = env.meta()->getJson(0)[sfAffectedNodes.fieldName];
