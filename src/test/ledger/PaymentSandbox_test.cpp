...
class PaymentSandbox_test : public beast::unit_test::suite
{
    ...
    */
    void testSelfFunding (std::initializer_list<uint256> fs)
    {
        ...
        env.fund (WFN (10000), snd, rcv, gw1, gw2);
    ...
    void testSubtractCredits (std::initializer_list<uint256> fs)
    {
        ...
        env.fund (WFN (10000), alice, gw1, gw2);
    ...
    void testReserve(std::initializer_list<uint256> fs)
    {
        ...
        auto accountFundsWFN = [&dj](
            ReadView const& view, AccountID const& id) -> WFNAmount
        {
            return toAmount<WFNAmount> (accountHolds (
                view, id, wfnCurrency (), wfnAccount (), fhZERO_IF_FROZEN, dj));
        };

        auto reserve = [](jtx::Env& env, std::uint32_t count) -> WFNAmount
        {
        ...
        PaymentSandbox sb (&av);
        {
            ...
            accountSend (sb, wfnAccount (), alice, WFN(100), dj);
            accountSend (sb, alice, wfnAccount (), WFN(100), dj);
            BEAST_EXPECT(accountFundsWFN (sb, alice) == beast::zero);
        }
    }
