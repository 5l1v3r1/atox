    ...
    : public beast::unit_test::suite
{
    ...
    testFlags()
    {
        ...
        env.trust(USD(100), alice, bob, carol);
        {
            ...
            env(offer(alice, WFN(5), USD(5)));

            // Now freeze gw.
            env(fset (gw, asfGlobalFreeze));
            env.close();
            env(offer(alice, WFN(4), USD(5)), ter(tecFROZEN));
            env.close();
        ...
        }
        {
            ...
            // carol's WFN balance should be her holdings minus her reserve.
            auto const carolsWFN = accountHolds (*env.closed(), carol,
                wfnCurrency(), wfnAccount(), fhZERO_IF_FROZEN, env.journal);
            // carol's WFN balance:              10000
            // base reserve:                      -200
            // 1 trust line times its reserve: 1 * -50
            //                                 -------
            // carol's available balance:         9750
            BEAST_EXPECT(carolsWFN == WFN(9750));

            // carol should be able to spend *more* than her WFN balance on
            // a fee by eating into her reserve.
            env(noop(carol), fee(carolsWFN + WFN(10)));
            env.close();

            // carol's WFN balance should now show as zero.
            BEAST_EXPECT(WFN(0) == accountHolds (*env.closed(),
                carol, wfnCurrency(), gw, fhZERO_IF_FROZEN, env.journal));
        }
        {
            // accountFunds().
    ...
    testAreCompatible()
    {
        ...
        eA.fund(WFN(10000), alice);
        eA.close();
        auto const rdViewA3 = eA.closed();

        eA.fund(WFN(10000), bob);
        eA.close();
        ...
        eB.fund(WFN(10000), bob);
        eB.close();
        auto const rdViewB3 = eB.closed();

        eB.fund(WFN(10000), alice);
        eB.close();
    ...
    testRegressions()
    {
        ...
        // Make sure OpenLedger::empty works
        {
            ...
            env.fund(WFN(10000), Account("test"));
            BEAST_EXPECT(! env.app().openLedger().empty());
        }
    }
