...
struct Directory_test : public beast::unit_test::suite
{
    ...
    void testDirectoryOrdering()
    {
...

        {
            ...
            env.fund(WFN(10000000), alice, gw);

            for (std::size_t i = 1; i <= 400; ++i)
                env(offer(alice, USD(i), WFN(i)));
            env.close();
            ...
            for (auto const& offer : book)
            {
                ...
                BEAST_EXPECT(offer->getFieldAmount(sfTakerGets) == WFN(count));
            }
        }
    }
    ...
    testDirIsEmpty()
    {
        ...
        env.fund(WFN(1000000), alice, charlie, gw);
        env.close();
    ...
    testRipd1353()
    {
        ...
        // should have no entries and be empty:
        {
            ...
            uint256 const bookBase = getBookBase({wfnIssue(), USD.issue()});
