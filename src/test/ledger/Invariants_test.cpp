...
class Invariants_test : public beast::unit_test::suite
{
    ...
    void
    testWFNNotCreated (bool enabled)
    {
        using namespace test::jtx;
        testcase << "checks " << (enabled ? "enabled" : "disabled") <<
            " - WFN created";
        doInvariantCheck (enabled,
            {{ "WFN net change was 500 on a fee of 0" }},
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                // put a single account in the view and "manufacture" some WFN
    ...
    testTypesMatch(bool enabled)
    {
        ...
        doInvariantCheck (enabled,
            {{ "ledger entry type mismatch" },
             { "WFN net change was -1000000000 on a fee of 0" }},
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
    ...
    void
    testNoWFNTrustLine(bool enabled)
    {
        using namespace test::jtx;
        testcase << "checks " << (enabled ? "enabled" : "disabled") <<
            " - trust lines with WFN not allowed";
        doInvariantCheck (enabled,
            {{ "an WFN trust line was created" }},
            [](Account const& A1, Account const& A2, ApplyContext& ac)
            {
                // create simple trust SLE with wfn currency
                auto index = getRippleStateIndex (A1, A2, wfnIssue().currency);
    ...
    void
    testWFNBalanceCheck(bool enabled)
    {
        using namespace test::jtx;
        testcase << "checks " << (enabled ? "enabled" : "disabled") <<
            " - WFN balance checks";
        ...
        doInvariantCheck (enabled,
            {{ "Cannot return non-native STAmount as WFNAmount" }},
            [](Account const& A1, Account const& A2, ApplyContext& ac)
            {
                //non-native balance
                auto const sle = ac.view().peek (keylet::account(A1.id()));
        ...
        doInvariantCheck (enabled,
            {{ "incorrect account WFN balance" },
             {  "WFN net change was 99999999000000001 on a fee of 0" }},
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
        ...
        doInvariantCheck (enabled,
            {{ "incorrect account WFN balance" },
             { "WFN net change was -1000000001 on a fee of 0" }},
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                // balance is negative
                auto const sle = ac.view().peek (keylet::account(A1.id()));
    ...
    testNoBadOffers(bool enabled)
    {
        ...
        doInvariantCheck (enabled,
            ...
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                ...
                sleNew->setFieldAmount (sfTakerPays, WFN(-1));
                ac.view().insert (sleNew);
                return true;
            });
        ...
        doInvariantCheck (enabled,
            ...
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                ...
                sleNew->setFieldAmount (sfTakerGets, WFN(-1));
                ac.view().insert (sleNew);
                return true;
            });
        ...
        doInvariantCheck (enabled,
            ...
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                ...
                sleNew->setAccountID (sfAccount, A1.id());
                sleNew->setFieldU32 (sfSequence, (*sle)[sfSequence]);
                sleNew->setFieldAmount (sfTakerPays, WFN(10));
                sleNew->setFieldAmount (sfTakerGets, WFN(11));
                ac.view().insert (sleNew);
                return true;
            });
    }
    ...
    testNoZeroEscrow(bool enabled)
    {
        ...
        doInvariantCheck (enabled,
            {{ "WFN net change was -1000000 on a fee of 0"},
            ...
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                ...
                auto sleNew = std::make_shared<SLE> (
                    keylet::escrow(A1, (*sle)[sfSequence] + 2));
                sleNew->setFieldAmount (sfAmount, WFN(-1));
                ac.view().insert (sleNew);
                return true;
            });

        doInvariantCheck (enabled,
            {{ "WFN net change was 100000000000000001 on a fee of 0" },
             {  "escrow specifies invalid amount" }},
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
...
public:
    void run ()
    {
        ...
        for(auto const& b : {false, true})
        {
            ...
            testTypesMatch (b);
            testNoWFNTrustLine (b);
            testWFNBalanceCheck (b);
            testNoBadOffers (b);
            testNoZeroEscrow (b);
        }
    }
