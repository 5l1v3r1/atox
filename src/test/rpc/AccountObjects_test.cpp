...
public:
    void testErrors()
    {
       ...
       // test errors on marker
        {
            Account const gw{ "G" };
            env.fund(WFN(1000), gw);
            auto const USD = gw["USD"];
            env.trust(USD(1000), bob);
            env(pay(gw, bob, WFN(1)));
            env(offer(bob, WFN(100), bob["USD"](1)), txflags(tfPassive));
    ...
    void testUnsteppedThenStepped()
    {
        ...
        env(offer(bob, WFN(100), bob["USD"](1)),txflags(tfPassive));
        env(offer(bob, WFN(100), USD1(1)), txflags(tfPassive));
