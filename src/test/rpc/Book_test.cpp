...
public:
    ...
    testOneSideEmptyBook()
    {
...

        {
            ...
            books[jss::books] = Json::arrayValue;
            {
                ...
                j[jss::snapshot] = true;
                j[jss::taker_gets][jss::currency] = "WFN";
...

        {
            ...
            env(offer("alice", WFN(700), USD(100)),
                ...
                [&](auto const& jv)
                {
                    ...
                    return t[jss::TransactionType] == "OfferCreate" &&
                        t[jss::TakerGets] == USD(100).value().getJson(0) &&
                            t[jss::TakerPays] == WFN(700).value().getJson(0);
                }));
        }

        {
            // Create a bid: TakerPays 100/USD, TakerGets 75
            env(offer("alice", USD(100), WFN(75)),
    ...
    testOneSideOffersInBook()
    {
        ...
        env(offer("alice", USD(100), WFN(200)),
            require(owners("alice", 2)));
        env.close();
...

        {
            ...
            BEAST_EXPECT(wsc->findMsg(5s,
                [&](auto const& jv)
                {
                    auto const& t = jv[jss::transaction];
                    return t[jss::TransactionType] == "OfferCreate" &&
                        t[jss::TakerGets] == WFN(75).value().getJson(0) &&
    ...
    testTrackOffers()
    {
        ...
        Account bob {"bob"};
        auto wsc = makeWSClient(env.app().config());
        env.fund(WFN(20000), alice, bob, gw);
        env.close();
        auto USD = gw["USD"];
        ...
        env(offer(alice, WFN(4000), USD(10)));
        env.close();

        Json::Value jvParams;
        jvParams[jss::taker] = env.master.human();
        jvParams[jss::taker_pays][jss::currency] = "WFN";
        ...
        BEAST_EXPECT(jrOffer[jss::TakerPays] == WFN(4000).value().getJson(0));
        ...
        BEAST_EXPECT(wsc->findMsg(5s,
            [&](auto const& jv)
            {
                ...
                return t[jss::TransactionType] == "OfferCreate" &&
                       ...
                       t[jss::TakerPays] == WFN(4000).value().getJson(0);
            }));

        env(offer(bob, WFN(2000), USD(5)));
        env.close();
        ...
        BEAST_EXPECT(wsc->findMsg(5s,
            [&](auto const& jv)
            {
                ...
                return t[jss::TransactionType] == "OfferCreate" &&
                       ...
                       t[jss::TakerPays] == WFN(2000).value().getJson(0);
            }));
        ...
        BEAST_EXPECT(jrNextOffer[jss::TakerPays] == WFN(2000).value().getJson(0));
    ...
    testCrossingSingleBookOffer()
    {
        ...
        // Scenario is:
        //  - Alice and Bob place identical offers for USD -> WFN
        ...
        env.fund (WFN(1000000), gw, alice, bob, charlie);
        env.close();
        ...
        // Alice and Bob offer $500 for 500 WFN
        env (offer (alice, WFN(500), USD(500)));
        env (offer (bob, WFN(500), USD(500)));
        env.close();
        ...
        env(offer(charlie, USD(1000), WFN(1000)));
        env.close();
        env.require(offers(alice, 0), offers(bob, 0), offers(charlie, 0));
        BEAST_EXPECT(offerOnlyOnceInStream(wsc, 1s, WFN(1000), USD(1000)));
    ...
    testCrossingMultiBookOffer()
    {
        ...
        // Scenario is:
        //  - Alice has 1 USD and wants 100 WFN
        //  - Bob has 100 WFN and wants 1 EUR
        ...
        env(offer(alice, WFN(100), USD(1)));
        env(offer(bob, EUR(1), WFN(100)));
        env.close();
...

        {
...

            {
                ...
                j[jss::taker_pays][jss::currency] = "WFN";
            }
    ...
    testBookOfferErrors()
    {
...

        {
            ...
            jvParams[jss::taker_gets][jss::currency] = "WFN";
            auto const jrr = env.rpc(
                "json", "book_offers", to_string(jvParams)) [jss::result];
...

        {
            ...
            BEAST_EXPECT(jrr[jss::error_message] ==
                ...
                "for WFN currency specification.");
        }

        {
            ...
            jvParams[jss::taker_pays][jss::issuer]   = toBase58(wfnAccount());
            jvParams[jss::taker_gets][jss::currency] = "USD";
            jvParams[jss::taker_gets][jss::issuer]   = gw.human();
            auto const jrr = env.rpc(
                "json", "book_offers", to_string(jvParams)) [jss::result];
            BEAST_EXPECT(jrr[jss::error] == "srcIsrMalformed");
            BEAST_EXPECT(jrr[jss::error_message] ==
                "Invalid field 'taker_pays.issuer', expected non-WFN issuer.");
        }
    ...
    testBookOfferLimits(bool asAdmin)
    {
        ...
        Account gw {"gw"};
        env.fund(WFN(200000), gw);
        env.close();
        auto USD = gw["USD"];

        for(auto i = 0; i <= RPC::Tuning::bookOffers.rmax; i++)
            env(offer(gw, WFN(50 + 1*i), USD(1.0 + 0.1*i)));
        env.close();
