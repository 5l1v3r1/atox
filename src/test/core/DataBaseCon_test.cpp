...
public:
	...
	void init_env() {
		...
		if (npos == std::string::npos) {
			std::cout << "usage: nschaind.exe --unittest=\"DataBaseConn\""
				<< "--unittest-arg=\"conf = nschaind.cfg\"" << std::endl;
			exit(-1);
		}

		try {
			nschain_conf_ = args.substr(npos + 4 + 1);
			std::cout << "conf = " << nschain_conf_ << std::endl;

			ripple::Config config;
			config.setup(nschain_conf_, true, false, false);
	...
	void test_InsertRecord() {
		ripple::DatabaseCon database(setup_, "ripple", NULL, 0, db_type_);
		soci::session& session = database.getSession();
		session << "insert into user (id,name) values (1,'nationalstonegroup')";
	...
	void test_UpdateRecord() {
		ripple::DatabaseCon database(setup_, "ripple", NULL, 0, db_type_);
		soci::session& session = database.getSession();
		std::string new_name = "new nationalstonegroup";
		int update_id = 1;
	...
	void test_SelectRecord() {
		...
		std::string expected_name[] = {"new nationalstonegroup", "ripple"};
