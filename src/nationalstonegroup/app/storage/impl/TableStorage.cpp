...
#include <nationalstonegroup/app/table/TableStatusDBMySQL.h>
#include <nationalstonegroup/app/table/TableStatusDBSQLite.h>
...
namespace ripple {
    ...
    {      
		...
		NSchainTx& nsChainTx = dynamic_cast<NSchainTx&>(transactor);
		...
		for (auto& eachTx : vecTxs)
		{
			...
			auto ret = TableStorageHandlePut(nsChainTx, uTxDBName, accountID, sTableName, lastLedgerSequence, tx.getTransactionID(), eachTx);
	...
	TER TableStorage::TableStorageHandlePut(NSchainTx& transactor,uint160 uTxDBName, AccountID accountID,std::string sTableName,uint32 lastLedgerSequence,uint256 txhash, STTx const & tx)
    {
        std::lock_guard<std::mutex> lock(mutexMap_);
