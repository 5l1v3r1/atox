...
#include <nationalstonegroup/app/tx/impl/Tuning.h>
#include <nationalstonegroup/app/tx/OperationRule.h>
...
namespace ripple {
        ...
        TableListSet::preflight(PreflightContext const& ctx)
    {
        ...
        tmpret = NSchainTx::preflight(ctx);
        if (!isTesSuccess(tmpret))
            return tmpret;
            ...
            if (tablesle)  //table exist
            {
                ...
                case T_CREATE:
                {
					...
					// Check reserve and funds availability
					{
						...
						STAmount priorBalance = STAmount((*sleAccount)[sfBalance]).wfn();
	...
	std::pair<TER, std::string> TableListSet::dispose(TxStore& txStore, const STTx& tx)
	{
		...
		return NSchainTx::dispose(txStore, tx);
	}
