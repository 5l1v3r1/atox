...
本文档包含了nschain的各个版本的注释。

关于nschain的编译方法，请参考 [编译](./Builds)。
关于如果运行nschain服务，请参考 [网络架设](http://www.nschain.net/setup.html)。

关于nschain的其它详细介绍，请访问 [www.nschain.net](http://www.nschain.net)。
...
- **表同步**：节点加入nschain链后可同步链上存在的数据库表，可同步加密（不加密）表到指定的时间、ledger或交易hash。
