...
'''
    ...
    install         Default target and copies it to build/nschaind (default)
...
GCC 5: If the gcc toolchain is used, gcc version 5 or better is required. On
    linux distros that ship with gcc 4 (ubuntu < 15.10), nschaind will force gcc
    to use gcc4's ABI (there was an ABI change between versions). This allows us
    to use the package manager to install nschaind dependencies. It also means if
...
Clang on linux: Clang cannot use the new gcc 5 ABI (clang does not know about
    ...
    link nschaind in this scenario (use the `--static` with scons), as dynamic
...
if (not platform.machine().endswith('64')):
    print('Warning: Detected {} architecture. nschaind requires a 64-bit OS.'.format(
          platform.machine()));
...
def check_openssl():
    ...
    if parse_time(d) < parse_time(build_time):
        raise Exception('Your openSSL was built on %s; '
                        'nschaind needs a version built on or after %s.'
                        % (line, build_time))
...
def get_classic_sources(toolchain):
    ...
    append_sources(result, *list_sources('src/nationalstonegroup/app', '.cpp'))
    append_sources(result, *list_sources('src/nationalstonegroup/basics', '.cpp'))
    append_sources(result, *list_sources('src/nationalstonegroup/crypto', '.cpp'))
    append_sources(result, *list_sources('src/nationalstonegroup/gmencrypt', '.cpp'))
    append_sources(result, *list_sources('src/nationalstonegroup/protocol', '.cpp'))
    append_sources(result, *list_sources('src/nationalstonegroup/rpc', '.cpp'))
...
def get_unity_sources(toolchain):
    ...
    append_sources(
        ...
        'src/nationalstonegroup/unity/gm_encrypt.cpp',
        'src/nationalstonegroup/unity/app_table.cpp',
        'src/nationalstonegroup/unity/app_sql.cpp',
...
for tu_style in ['classic', 'unity']:
    for toolchain in all_toolchains:
        for variant in variants:
            ...
            target = env.Program(
                target=os.path.join(variant_dir, 'nschaind'),
...
vcxproj = base.VSProject(
    os.path.join('Builds', 'VisualStudio2015', 'nschaind'),
    source = [],
    VSPROJECT_ROOT_DIRS = [
        'build/',
        'src/beast/extras',
...
# Build the list of nschaind source files that hold unit tests
